...
export function initDB(dataDir: string): Database.Database {
  ...
  try { db.exec('ALTER TABLE gc_messages ADD COLUMN thinking TEXT') } catch {}
  try { db.exec('ALTER TABLE gc_messages ADD COLUMN tool_calls TEXT') } catch {}
...
export function validateNs(ns: string): string {
  const allowed = ['agents', 'providers', 'models', 'skills', 'mcp',
    'groups', 'conversations', 'config', 'settings', 'drafts', 'prompts',
    'documents', 'workflows', 'rag', 'rag_history',
    'generated_files', 'exec_logs', 'feedback_log', 'error_patterns', 'habits',
    'calendar', 'goals', 'permissions', 'templates', 'distilled_skills',
    'checkpoints', 'messages', 'memory_recycle', 'notes']
...
export function gcMsgList(groupId: string) {
  validateId(groupId)
  return getDB().prepare('SELECT id, group_id as groupId, sender_id as senderId, sender_name as senderName, role, content, timestamp, bookmarked, pinned, thinking, tool_calls as toolCalls FROM gc_messages WHERE group_id = ? ORDER BY rowid').all(groupId)
}

export function gcMsgAdd(groupId: string, senderId: string, senderName: string, role: string, content: string, thinking?: string, toolCalls?: string) {
  validateId(groupId)
  const id = 'gm-' + Date.now() + '-' + randomUUID().slice(0, 8)
  getDB().prepare('INSERT INTO gc_messages (id, group_id, sender_id, sender_name, role, content, thinking, tool_calls) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(id, groupId, senderId, senderName, role, content, thinking || null, toolCalls || null)
  return id
}
