...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  // Allow any model name — pass through to provider routing
  // Original: reject non-openclaw model names
  ...
  // Use x-openclaw-model header, or fall back to request body model
  const modelSource = raw || requestModel;
  if (!modelSource) {
    return {};
  }
  ...
  const parsed = parseModelRef(modelSource, defaultProvider, {
