...
function writeAssistantReasoningChunk(
  res: ServerResponse,
...
export async function handleOpenAiHttpRequest(
  ...
  const unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "thinking") {
      ...
      writeAssistantReasoningChunk(res, {
        runId,
        model,
        content: delta,
      });
    ...
    // Forward tool execution events to SSE stream for client-side UI
    if (evt.stream === "tool") {
      const data = evt.data ?? {};
          ...
          choices: [{
            delta: {
              tool_event: {
                ...
                error: phase === "result" && data.isError ? (typeof data.result === 'string' ? data.result : JSON.stringify(data.result)) : undefined,
              }
            }
          }]
        });
