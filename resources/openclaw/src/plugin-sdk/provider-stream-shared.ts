...
export function createDeepSeekV4OpenAICompatibleThinkingWrapper(params: {
  ...
  return (model, context, options) => {
    ...
    const effort = disabled ? undefined : resolveReasoningEffort(params.thinkingLevel);

    // Set reasoning on options so downstream streamSimpleOpenAICompletions converts it
    // to reasoningEffort, which streamOpenAICompletions needs for shouldEmitReasoning=true
    // to emit reasoning_content from the SSE stream.
