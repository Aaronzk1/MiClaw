...
> = (model: Model<"openai-completions">, context: Context, options?: OpenAICompletionsOptions) => {
  ...
  void (async () => {
    ...
    try {
      ...
      // Parse <tool_call> blocks from text content for models without native function calling
      // This handles models that output tool calls as XML in their text response
          ...
          while ((match = toolCallRegex.exec(text)) !== null) {
            try {
              ...
              if (parsed.name) {
                ...
                // Remove the <tool_call> text from the text block
                block.text = block.text.replace(match[0], "").trim();
              }
            } catch {}
          }
        }
        // Remove empty text blocks after extraction
        output.content = output.content.filter(
          (b) => b.type !== "text" || (b as TextContent).text.trim().length > 0,
        );
      }
...
export function convertMessages(
  ...
  if (context.systemPrompt) {
    ...
    let systemPrompt = options.preserveSystemPromptCacheBoundary
      ? context.systemPrompt
      : stripSystemPromptCacheBoundary(context.systemPrompt);
    // Inject tool descriptions into system prompt for models without native function calling support.
    // Providers with native support (openai, anthropic, google, mistral, groq, together, openrouter,
    // github-copilot, deepseek, cohere, fireworks, perplexity, zai) use the API `tools` parameter.
    // All other providers (MiMo, Qwen, local models, etc.) get XML tool descriptions as fallback.
    if (context.tools && context.tools.length > 0) {
      const nativeProviders = new Set([
        "openai", "anthropic", "google", "mistral", "groq", "together", "openrouter",
        "github-copilot", "deepseek", "cohere", "fireworks", "perplexity", "zai",
      ]);
      if (!nativeProviders.has(model.provider)) {
        systemPrompt += toolsToXmlPrompt(context.tools);
      }
    }
    params.push({
      role,
...
/**
 * Convert tools to XML format for injection into system prompt.
 * This enables models that don't support native function calling (e.g. MiMo, Qwen, DeepSeek)
 * to still invoke tools by outputting <tool_call> XML blocks.
 */
function toolsToXmlPrompt(tools: Tool[]): string {
  if (!tools.length) return "";
  const lines = [
    "\n\n# Available Tools",
    "You have access to the following tools. To call a tool, output a <tool_call> block in your response.",
    "Format: <tool_call>{\"name\": \"tool_name\", \"arguments\": {\"key\": \"value\"}}</tool_call>",
    "You may call multiple tools in a single response. Wait for tool results before continuing.\n",
  ];
    ...
    if (tool.parameters && typeof tool.parameters === "object") {
      ...
      if (params.properties) {
        ...
        for (const [key, prop] of Object.entries(properties)) {
          ...
          lines.push(`    <param name="${key}" type="${type}${req}">${desc}</param>`);
        }
        lines.push(`  </parameters>`);
      }
    }
...
function detectCompat(model: Model<"openai-completions">): ResolvedOpenAICompletionsCompat {
  ...
  const useMaxTokens =
    baseUrl.includes("chutes.ai") || isMoonshot || isCloudflareAiGateway || isTogether || isXiaomi;
