import { a as redactSensitiveFieldValue, c as redactSensitiveText, i as redactSecrets, n as getDefaultRedactPatterns, p as readLoggingConfig } from "./redact-DduLliKq.js";
...
import { a as resolveSessionFilePathOptions, i as resolveSessionFilePath, o as resolveSessionTranscriptPath, r as resolveDefaultSessionStorePath } from "./paths-NEwU8m3X.js";
import { A as parseSessionThreadInfo, V as resolveSessionStoreEntry, t as loadSessionStore } from "./store-load-C4uq6Lrq.js";
...
import { a as writeJsonlEntry, o as writeJsonlLines, r as serializeJsonlLine, t as appendJsonlEntry } from "./transcript-jsonl-CD0KPCG0.js";
...
function redactTranscriptStructuredValue(value, cfg, fieldKey, seen = /* @__PURE__ */ new WeakSet(), preserveImageDataUrlFields = false) {
	...
	if (Array.isArray(value)) {
		...
		const redacted = value.map((item) => {
			const next = redactTranscriptStructuredValue(item, cfg, fieldKey, seen, preserveImageDataUrlFields);
			changed ||= next !== item;
			return next;
		});
		seen.delete(value);
	...
	for (const [key, item] of Object.entries(source)) {
		...
		const redacted = redactTranscriptStructuredValue(item, cfg, key, seen, preserveImageDataUrlFields || shouldPreserveNestedImageDataUrlFields(source, key));
...
async function ensureTranscriptHeader(transcriptPath, params = {}) {
	...
	await writeJsonlEntry(transcriptPath, createSessionTranscriptHeader(params), {
		mode: 384,
...
async function ensureSessionHeader(params) {
	...
	await writeJsonlEntry(params.sessionFile, header, { mode: 384 });
}
...
async function appendExactAssistantMessageToSessionTranscript(params) {
	...
	if (result.ok && transcriptMarkerUpdatedAt !== void 0) await updateSessionStoreEntry({
		storePath,
		sessionKey: resolved.normalizedKey,
		update: (current) => current.sessionId === entry.sessionId ? { updatedAt: transcriptMarkerUpdatedAt } : null
	});
	return result;
}
...
//#endregion
export { resolveSessionTranscriptFile as a, bindOwnedSessionTranscriptWrites as c, resolveAndPersistSessionFile as d, isOpenClawDeliveryMirrorAssistantMessage as f, redactTranscriptMessage as h, readTailAssistantTextFromSessionTranscript as i, withOwnedSessionTranscriptWrites as l, isTranscriptOnlyOpenClawAssistantModel as m, appendExactAssistantMessageToSessionTranscript as n, resolveMirroredTranscriptText as o, isTranscriptOnlyOpenClawAssistantMessage$1 as p, readLatestAssistantTextFromSessionTranscript as r, appendSessionTranscriptMessage as s, appendAssistantMessageToSessionTranscript as t, streamSessionTranscriptLines as u };
