...
function writeAssistantReasoningChunk(res, params) {
	writeSse(res, {
		id: params.runId,
		object: "chat.completion.chunk",
		created: Math.floor(Date.now() / 1e3),
		...
		choices: [{
			index: 0,
			delta: { reasoning_content: params.content },
			finish_reason: null
		}]
	});
}
	...
	const unsubscribe = onAgentEvent((evt) => {
		...
		if (evt.stream === "thinking") {
			...
			if (delta) {
				...
				writeAssistantReasoningChunk(res, { runId, model, content: delta });
			}
			return;
		}
			...
			if (toolCallId && (phase === "start" || phase === "result")) {
				writeSse(res, {
					choices: [{
						delta: {
							tool_event: {
								phase, toolCallId, name: toolName,
								status: phase === "start" ? "running" : (data.isError ? "error" : "done"),
								args: phase === "start" ? data.args : undefined,
								error: phase === "result" && data.isError ? (data.error?.message || data.error || "tool error") : undefined,
								output: phase === "result" && !data.isError ? data.output : undefined,
							}
						}
					}]
				});
