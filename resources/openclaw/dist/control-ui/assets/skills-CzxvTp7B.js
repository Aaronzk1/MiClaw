import{a as e,m as t,o as n,p as r,r as i}from"./lit-runtime-BImxIzGR.js";import{o as a,t as o}from"./string-coerce-Dt6Kt5Fx.js";import{P as s,R as c,_ as l,g as u}from"./index-B6YtUsIL.js";import{i as d,n as f,r as p,t as m}from"./skills-shared-CNlkuf6o.js";function h(e){return e?u(e,window.location.href):null}function g(e){!(e instanceof HTMLDialogElement)||e.open||(e.isConnected?e.showModal():queueMicrotask(()=>{e.isConnected&&!e.open&&e.showModal()}))}var _=[{id:`all`,label:`All`},{id:`ready`,label:`Ready`},{id:`needs-setup`,label:`Needs Setup`},{id:`disabled`,label:`Disabled`}];function v(e,t){switch(t){case`all`:return!0;case`ready`:return!e.disabled&&e.eligible;case`needs-setup`:return!e.disabled&&!e.eligible;case`disabled`:return e.disabled}throw Error(`Unsupported skills status filter`)}function y(e){return e.disabled?`muted`:e.eligible?`ok`:`warn`}function b(e,t){let n=e.clawhub;return!n||n.status!==`linked`||!n.valid?null:t[s({registry:n.registry,slug:n.slug,version:n.installedVersion})]??null}function x(e){if(!e)return`Unavailable`;let t=e.securityStatus?.trim()||null;return e.ok&&e.decision===`pass`?t===`clean`||!t?`Clean`:t:t===`pending`||t===`not-run`?`Pending`:t===`malicious`?`Blocked`:t===`suspicious`?`Review`:`Unavailable`}function S(e){if(!e)return`chip-warn`;if(e.ok&&e.decision===`pass`)return`chip-ok`;let t=e.securityStatus?.trim()||null;return t===`pending`||t===`not-run`?`chip`:`chip-warn`}function C(n){let i=n.report?.skills??[],s={all:i.length,ready:0,"needs-setup":0,disabled:0};for(let e of i)e.disabled?s.disabled++:e.eligible?s.ready++:s[`needs-setup`]++;let c=n.statusFilter===`all`?i:i.filter(e=>v(e,n.statusFilter)),l=o(n.filter),u=l?c.filter(e=>o([e.name,e.description,e.source].join(` `)).includes(l)):c,f=d(u),p=n.detailKey?i.find(e=>e.skillKey===n.detailKey)??null:null;return t`
    <section class="card">
      ...
      <div class="agent-tabs" style="margin-top: 14px;">
        ${_.map(e=>t`
            ...
            >
              ${e.label}<span class="agent-tab-count">${s[e.id]}</span>
            </button>
          `)}
      </div>
              ...
              ${f.map(r=>t`
                  <details class="agent-skills-group" open>
                    ...
                    <div class="list skills-grid">
                      ${e(r.skills,e=>e.skillKey,e=>E(e,n))}
                    </div>
                  </details>
                `)}
            </div>
    ...
    ${p?D(p,n):r}
    ${n.clawhubDetailSlug?T(n):r}
  `}function w(e){let n=e.clawhubResults;return n?n.length===0?t`<div class="muted" style="margin-top: 8px;">No skills found on ClawHub.</div>`:t`
      ...
      ${n.map(n=>t`
          ...
          >
            <div class="list-main">
              ...
              <div class="list-sub">${n.summary?c(n.summary,120):n.slug}</div>
            </div>
  ...
  `:r}function T(e){let i=e.clawhubDetail;return t`
    <dialog
  ...
  `}function E(e,n){let i=n.busyKey===e.skillKey,a=y(e),o=b(e,n.clawhubVerdicts);return t`
    <div class="list-item list-item-clickable" @click=${()=>n.onDetailOpen(e.skillKey)}>
      <div class="list-main">
        ...
        <div class="list-sub">${c(e.description,140)}</div>
      </div>
      <div
        class="list-meta"
        style="display: flex; align-items: center; justify-content: flex-end; gap: 10px;"
      >
        ${e.clawhub?.status===`linked`?t`<span class="chip ${S(o)}">${x(o)}</span>`:e.clawhub?.status===`invalid`?t`<span class="chip chip-warn">ClawHub link invalid</span>`:r}
  ...
  `}function D(e,i){let a=i.busyKey===e.skillKey,o=i.edits[e.skillKey]??``,s=i.messages[e.skillKey]??null,c=e.install.length>0&&e.missing.bins.length>0,l=!!(e.bundled&&e.source!==`openclaw-bundled`),u=m(e),d=f(e),_=b(e,i.clawhubVerdicts),v=i.detailTab===`card`&&e.skillCard?.present?`card`:`overview`;return t`
    <dialog
      class="md-preview-dialog"
      ${n(g)}
      @click=${e=>{let t=e.currentTarget;e.target===t&&t.close()}}
        ...
        <div class="md-preview-dialog__body" style="display: grid; gap: 16px;">
          ...
          ${e.primaryEnv?t`
                <div style="display: grid; gap: 8px;">
                  ...
                  ${(()=>{let n=h(e.homepage);return n?t`<div class="muted" style="font-size: 13px;">
                          Get your key:
          ...
          >
            ...
            ${(()=>{let n=h(e.homepage);return n?t`<div>
                    <a href="${n}" target="_blank" rel="noopener noreferrer"
                      >${e.homepage}</a
                    >
                  </div>`:r})()}
  ...
  `}function O(e,n,i){let a=e.clawhub;if(!a)return r;if(a.status===`invalid`)return t`<div class="callout danger">
      <div style="font-weight: 600; margin-bottom: 4px;">ClawHub link invalid</div>
      <div>${a.reason}</div>
    </div>`;let o=h(i?.securityAuditUrl??void 0),s=i?.reasons?.length?i.reasons.join(`, `):null;return t`
    <div
      class="callout"
      style="display: grid; gap: 8px; border-color: var(--border); background: var(--panel-2);"
    >
      <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
        <span class="chip ${S(i)}">${x(i)}</span>
  ...
  `}function k(e,n){if(!e.skillCard?.present)return r;let a=n.skillCardContents[e.skillKey];if(a===void 0){let r=n.skillCardErrors[e.skillKey];return r?t`<div class="callout danger">${r}</div>`:t`<div class="muted" style="font-size: 13px;">
  ...
  `}export{C as renderSkills};
//# sourceMappingURL=skills-CzxvTp7B.js.map