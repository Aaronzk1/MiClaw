...
function convertMessages(model, context, compat, options = {}) {
	...
	if (context.systemPrompt) {
		...
		let systemPrompt = options.preserveSystemPromptCacheBoundary ? context.systemPrompt : stripSystemPromptCacheBoundary(context.systemPrompt);
		// Inject tool descriptions for models without native function calling
		if (context.tools && context.tools.length > 0) {
			const nativeProviders = new Set(["openai", "anthropic", "google", "mistral", "groq", "together", "openrouter", "github-copilot", "deepseek", "cohere", "fireworks", "perplexity", "zai"]);
...
function toolsToXmlPrompt(tools) {
...
function detectCompat(model) {
	...
	const isNonStandard = provider === "cerebras" || baseUrl.includes("cerebras.ai") || provider === "xai" || baseUrl.includes("api.x.ai") || isTogether || baseUrl.includes("chutes.ai") || baseUrl.includes("deepseek.com") || isZai || isMoonshot || provider === "opencode" || baseUrl.includes("opencode.ai") || isCloudflareWorkersAI || isCloudflareAiGateway || isXiaomi;
	const useMaxTokens = baseUrl.includes("chutes.ai") || isMoonshot || isCloudflareAiGateway || isTogether || isXiaomi;
