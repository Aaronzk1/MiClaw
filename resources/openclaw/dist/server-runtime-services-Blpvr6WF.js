import { n as isVitestRuntimeEnv } from "./env-Di49gJwo.js";
import { t as isGatewayModelPricingEnabled } from "./model-pricing-config-pKJPI_G6.js";
import { r as startHeartbeatRunner } from "./heartbeat-runner-7zBJym-X.js";
import { r as createNoopHeartbeatRunner } from "./server-runtime-startup-services-B_m8NSaC.js";
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-BoyNluFA.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-DD0U1vwf.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-vz7ikyhF.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-BvL6wIT1.js");
		if (stopped) return;
