...
function buildParams(model, context, isOAuthTokenResult, options) {
	...
	const compat = context.tools?.length ? getAnthropicCompat(model) : void 0;
	const tools = context.tools && compat ? convertTools(context.tools, isOAuthTokenResult, compat.supportsEagerToolInputStreaming, compat.supportsCacheControlOnTools ? cacheControl : void 0) : void 0;
	...
	const params = {
		model: model.id,
		messages: convertMessages(context.messages, model, isOAuthTokenResult, cacheControl, messageCacheControlLimit),
...
function convertMessages(messages, model, isOAuthTokenValue, cacheControl, messageCacheControlLimit = 4) {
	const params = [];
