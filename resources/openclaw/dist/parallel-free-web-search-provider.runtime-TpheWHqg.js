...
import { S as writeCachedSearchPayload, _ as resolveSearchTimeoutSeconds, f as readCachedSearchPayload, h as resolveSearchCacheTtlMs, x as withTrustedWebSearchEndpoint } from "./web-search-provider-common-DisZTXTb.js";
...
import { a as normalizeParallelClientModel, c as normalizeParallelSearchQueries, d as stripParallelGeneratedSessionId, i as mapParallelResults, l as normalizeParallelSessionId, n as buildParallelCacheKey, o as normalizeParallelObjective, r as invalidSearchQueriesPayload, u as resolveParallelSearchCount } from "./parallel-search-normalize-BrqPQAw_.js";
...
function extractMcpToolPayload(envelope) {
	if ("error" in envelope) throw new Error(`Parallel MCP error: ${JSON.stringify(envelope.error).slice(0, 500)}`);
	const result = isRecord(envelope.result) ? envelope.result : {};
	if (result.isError) throw new Error(`Parallel MCP tool error: ${JSON.stringify(result).slice(0, 500)}`);
	...
	throw new Error(`Parallel MCP returned no parseable content: ${JSON.stringify(result).slice(0, 500)}`);
}
...
async function executeParallelFreeWebSearchProviderTool(ctx, args, signal) {
	...
	const sessionId = normalizeParallelSessionId(readStringParam(args, "session_id"), 100);
