...
const loadOverviewModule = async () => await import("./overview-BAGE25yt.js");
const loadModelsSharedModule = async () => await import("./shared-DyBfRrDF.js");
const loadConfigCliModule = async () => await import("./config-cli-DC7j84k_.js");
const loadDoctorModule = async () => await import("./doctor-DMdCGvR-.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-DN9r7mpZ.js");
	if (operation === "start") {
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-search") {
		...
		await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
			const { runPluginsSearchCommand } = await import("./plugins-search-command-C_J1DMEQ.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-CciI5qPZ.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-_8XCCPCg.js")).agentsAddCommand)({
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-BIWnckoj.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-C57_V_VY.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
