 ...
 */
export function kindFromStateKey(stateKey) {
    if (stateKey.startsWith("pipeline_resume_"))
        return "pipeline-resume";
    if (stateKey.startsWith("workflow_resume_"))
...
export function parseResumeArgs(argv) {
    ...
    if (!args.token && !args.approvalId)
        throw new Error("resume requires --token or --id");
 ...
 */
export async function resolveApprovalId(approvalId, env) {
