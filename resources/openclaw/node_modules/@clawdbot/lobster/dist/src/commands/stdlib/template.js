...
function splitFilterChain(expr) {
    const parts = [];
    let current = "";
    let i = 0;
    while (i < expr.length) {
...
export const templateCommand = {
    ...
    meta: {
        ...
        argsSchema: {
            ...
            properties: {
                text: {
                    type: "string",
                    description: "Template text (supports {{path}}, {{path | filter}}, {{.}} for the whole item)",
                },
    ...
    async run({ input, args }) {
        ...
        if (!tpl)
            throw new Error("template requires --text or --file (or positional text)");
        return {
