...
export const emailTriageCommand = {
    ...
    async run({ input, args, ctx }) {
        ...
        const hasLlmProvider = Boolean(String(env.LOBSTER_LLM_PROVIDER ?? "").trim() ||
        ...
        if (!ctx?.registry)
            throw new Error("email.triage (LLM mode) requires ctx.registry");
        const llmCmd = ctx.registry.get("llm.invoke") ?? ctx.registry.get("llm_task.invoke");
        if (!llmCmd)
            throw new Error("email.triage requires llm.invoke to be registered");
        ...
        const llmItems = [];
        for await (const it of llmRes.output)
            llmItems.push(it);
        ...
        for (const d of decisions) {
            if (d.category === "needs_reply")
                buckets.needsReply.push(d.id);
            else if (d.category === "needs_action")
                buckets.needsAction.push(d.id);
            else
                buckets.fyi.push(d.id);
...
//# sourceMappingURL=email_triage.js.map