...
export function prMonitor(options) {
    ...
    if (!repo)
        throw new Error("prMonitor requires repo");
    if (!pr)
        throw new Error("prMonitor requires pr");
...
export function prMonitorNotify(options) {
    ...
    const key = options.key ?? `github.pr:${repo}#${pr}`;
    if (!repo)
        throw new Error("prMonitorNotify requires repo");
    if (!pr)
        throw new Error("prMonitorNotify requires pr");
    const workflow = new Lobster()
        .pipe(ghPrView({ repo, pr }))
