...
test("on_error validation rejects unsupported values", async () => {
    ...
    await assert.rejects(() => loadWorkflowFile(filePath), /on_error must be "stop", "continue", or "skip_rest"/);
});
...
test("external abort still propagates when timeout is configured", async () => {
    ...
    }, { signal: controller.signal }), (err) => err?.name === "AbortError" || err?.code === "ABORT_ERR");
});
...
//# sourceMappingURL=step_timeout.test.js.map