...
test("sdk Lobster.resume rejects invalid structured input responses", async () => {
    ...
    const first = await workflow.run();
    assert.equal(first.status, "needs_input");
    await assert.rejects(() => workflow.resume(first.requiresInput.resumeToken, { response: { decision: "maybe" } }), /does not match schema/i);
});
//# sourceMappingURL=sdk_lobster.test.js.map