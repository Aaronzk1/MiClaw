...
test("approve halts pipeline in tool mode", async () => {
    ...
    const pipeline = parsePipeline("exec --json --shell \"node -e 'process.stdout.write(JSON.stringify([{a:1}]))'\" | approve --prompt 'send?' | exec --shell 'exit 1'");
...
test("approve passes through in human interactive mode only (emit required otherwise)", async () => {
    ...
    const items = [];
    for await (const it of result.output)
        items.push(it);
...
//# sourceMappingURL=tool_mode.test.js.map