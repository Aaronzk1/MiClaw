...
async function setupWorkflows(files) {
...
async function runWorkflow(filePath, stateDir, args) {
...
test("workflow validation rejects workflow combined with run", async () => {
    ...
    await assert.rejects(() => loadWorkflowFile(filePath), /can only define one of run, command, pipeline, workflow, parallel, or for_each/);
});
...
test("sub-workflow approval gates are rejected in composition", async () => {
    ...
    await assert.rejects(() => runWorkflow(paths["parent.lobster"], stateDir), /Sub-workflow approval\/input gates are not supported in composition/);
});
//# sourceMappingURL=workflow_composition.test.js.map