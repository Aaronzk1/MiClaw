...
test("withRetry throws after max exhausted", async () => {
    ...
    }, resolveRetryConfig({ max: 2, delay_ms: 10 })), /always fails/);
...
test("withRetry never retries abort errors", async () => {
    ...
    await assert.rejects(withRetry(async () => {
        calls++;
        throw abortErr;
    }, resolveRetryConfig({ max: 3, delay_ms: 10 })), (err) => err.name === "AbortError");
    assert.equal(calls, 1);
});
test("withRetry calls onRetry callback", async () => {
    ...
    await withRetry(async () => {
        ...
        return "ok";
    }, resolveRetryConfig({ max: 3, delay_ms: 10 }), { onRetry: (attempt) => retries.push(attempt) });
...
test("retry validation rejects non-object", async () => {
    await assert.rejects(loadWorkflow({ name: "bad", steps: [{ id: "x", command: "echo", retry: "yes" }] }), /retry must be an object/);
});
test("retry validation rejects non-integer max", async () => {
    await assert.rejects(loadWorkflow({ name: "bad", steps: [{ id: "x", command: "echo", retry: { max: 1.5 } }] }), /retry.max must be a positive integer/);
});
test("retry validation rejects max < 1", async () => {
    await assert.rejects(loadWorkflow({ name: "bad", steps: [{ id: "x", command: "echo", retry: { max: 0 } }] }), /retry.max must be a positive integer/);
});
test("retry validation rejects invalid backoff", async () => {
    await assert.rejects(loadWorkflow({
        name: "bad",
...
test("retry validation rejects negative delay_ms", async () => {
    await assert.rejects(loadWorkflow({ name: "bad", steps: [{ id: "x", command: "echo", retry: { delay_ms: -1 } }] }), /retry.delay_ms must be a finite non-negative number/);
});
test("retry validation rejects non-boolean jitter", async () => {
    await assert.rejects(loadWorkflow({ name: "bad", steps: [{ id: "x", command: "echo", retry: { jitter: "yes" } }] }), /retry.jitter must be a boolean/);
});
...
async function runWorkflow(workflow) {
    ...
    stderr.on("data", (d) => stderrChunks.push(d.toString()));
...
test("step exhausts retries and throws", async () => {
    ...
    await assert.rejects(runWorkflow(workflow).then((r) => r.result), /workflow command failed/);
});
...
//# sourceMappingURL=step_retry.test.js.map