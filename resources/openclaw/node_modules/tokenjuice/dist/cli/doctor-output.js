...
function getIntegrationPath(report) {
    return (getStringField(report, "hooksPath") ??
        ...
        getStringField(report, "memoryPath") ??
...
export function formatHookDoctorReport(report) {
    ...
    for (const [index, [name, integrationReport]] of installedIntegrations.entries()) {
        ...
        if ("runtimeConfig" in integrationReport && integrationReport.runtimeConfig?.configExists) {
            ...
            lines.push(`- codex config: approval_policy=${runtime.approvalPolicy ?? "(default)"}, sandbox_mode=${runtime.sandboxMode ?? "(default)"}, approvals_reviewer=${runtime.approvalsReviewer ?? "(default)"}`);
        }
