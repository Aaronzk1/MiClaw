...
import { doctorCopilotCliHook, getCopilotCliInstructionsSnippet, installCopilotCliHook, runCopilotCliPostToolUseHook, uninstallCopilotCliHook, } from "../hosts/copilot-cli/index.js";
...
import { doctorOpenCodeExtension, installOpenCodeExtension, uninstallOpenCodeExtension, } from "../hosts/opencode/index.js";
...
import { doctorVscodeCopilotHook, getVscodeCopilotInstructionsSnippet, installVscodeCopilotHook, runVscodeCopilotPreToolUseHook, uninstallVscodeCopilotHook, } from "../hosts/vscode-copilot/index.js";
...
function formatPlandexLoadCommand(conventionPath) {
...
async function readStdin(maxBytes = DEFAULT_MAX_INPUT_BYTES) {
    if (inputStdin.isTTY) {
        return "";
    }
    const chunks = [];
...
async function readTextInput(file, maxBytes = DEFAULT_MAX_INPUT_BYTES) {
    if (!file) {
...
function emit(format, value, text, exactText = false) {
    if (format === "json") {
...
async function runReduceJson(args) {
    const file = args.positionals[0];
    const rawText = await readTextInput(file, args.maxInputBytes);
    const noOmit = resolveNoOmit(args.noOmit);
    if (!rawText.trim()) {
    ...
    const request = parseReduceJsonRequest(JSON.parse(rawText));
...
export function resolveNoOmit(noOmitFlag, env = process.env) {
    return noOmitFlag || readNoOmissionFromEnv(env);
}
export function decorateWrapInlineText(result, raw) {
...
async function runVerify(args) {
    ...
    if (args.format === "json") {
        process.stdout.write(`${JSON.stringify(args.fixtures ? { rules: results, fixtures: fixtureResults } : results, null, 2)}\n`);
    ...
    if (failed.length === 0 && failedFixtures.length === 0) {
        ...
        process.stdout.write(`ok: ${results.length} rules validated${warned.length > 0 ? `, ${warned.length} warnings` : ""}${args.fixtures ? `, ${fixtureResults.length} fixtures verified` : ""}\n`);
        return 0;
    }
...
async function loadDirectAnalysisEntry(args) {
    const file = args.positionals[0];
...
async function runDoctor(args) {
    ...
    if (args.positionals[0] === "codex") {
        ...
        if (report.featureFlag.enabled) {
            ...
            process.stdout.write(`feature flag: hooks enabled via ${source} (${report.featureFlag.configPath})\n`);
        }
        else {
        ...
        if (report.runtimeConfig.configExists) {
            process.stdout.write(`codex config: approval_policy=${report.runtimeConfig.approvalPolicy ?? "(default)"}, sandbox_mode=${report.runtimeConfig.sandboxMode ?? "(default)"}, approvals_reviewer=${report.runtimeConfig.approvalsReviewer ?? "(default)"}\n`);
        }
        process.stdout.write(`repair: ${report.fixCommand}\n`);
        return report.status === "broken" ? 1 : 0;
    }
    ...
    if (report.topWeakReducers.length > 0) {
        ...
        for (const candidate of report.topWeakReducers.slice(0, 5)) {
            process.stdout.write(`- ${candidate.signature} reducer=${candidate.matchedReducer ?? "n/a"} count=${formatMetric(candidate.count)} avgRatio=${formatRatio(candidate.avgRatio)}\n`);
        }
    }
...
async function runStats(args) {
    ...
    if (report.reducers.length > 0) {
        ...
        for (const reducer of report.reducers.slice(0, 5)) {
            process.stdout.write(`- ${reducer.reducer} count=${formatMetric(reducer.count)} saved=${formatMetric(reducer.savedChars)} avgRatio=${formatRatio(reducer.avgRatio)}\n`);
        }
    }
    if (report.commands.length > 0) {
        process.stdout.write("top commands:\n");
        for (const command of report.commands.slice(0, 5)) {
            process.stdout.write(`- ${command.signature} count=${formatMetric(command.count)} saved=${formatMetric(command.savedChars)} avgRatio=${formatRatio(command.avgRatio)}\n`);
        }
    }
    ...
    if (report.sources && report.sources.length > 0) {
        ...
        for (const source of report.sources) {
            process.stdout.write(`source ${source.source}: entries=${formatMetric(source.totals.entries)} saved=${formatMetric(source.totals.savedChars)} avgRatio=${formatRatio(source.totals.avgRatio)}\n`);
            if (source.reducers.length > 0) {
                process.stdout.write(`  reducers: ${source.reducers.slice(0, 3).map((reducer) => `${reducer.reducer}(${formatMetric(reducer.count)})`).join(", ")}\n`);
            }
            if (source.commands.length > 0) {
                process.stdout.write(`  commands: ${source.commands.slice(0, 3).map((command) => `${command.signature}(${formatMetric(command.count)})`).join(", ")}\n`);
            }
        }
    }
    return 0;
...
export async function isDirectModuleEntrypoint(moduleUrl, argv = process.argv) {
...
async function runCliIfEntrypoint() {
