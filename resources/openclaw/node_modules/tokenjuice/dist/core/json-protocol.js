...
function validateOptionalRecord(value, path, errors) {
    if (value !== undefined && !isRecord(value)) {
        errors.push(`${path} must be an object`);
    }
}
function validateToolExecutionInput(raw) {
    const errors = [];
    if (!isRecord(raw)) {
        return {
            ok: false,
...
function validateReduceOptions(raw) {
    if (raw === undefined) {
        return {
            ok: true,
            value: {},
...
export function parseReduceJsonRequest(raw) {
    if (!isRecord(raw)) {
