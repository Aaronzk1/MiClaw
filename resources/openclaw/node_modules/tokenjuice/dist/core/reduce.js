...
import { NO_COMPACTION_METADATA, mergeCompactionMetadata } from "./compaction-metadata.js";
...
export async function reduceExecution(input, opts = {}) {
    const rules = await loadRules(opts.cwd ? { cwd: opts.cwd } : undefined);
    return reduceExecutionWithRules(input, rules, opts);
}
export async function reduceExecutionWithRules(input, rules, opts = {}) {
...
export async function classifyOnly(input, forcedRuleId) {
    const rules = await loadRules();
    return classifyExecution(input, rules, forcedRuleId);
}
export async function findMatchingRule(input) {
    const rules = await loadRules();
