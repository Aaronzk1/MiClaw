import { createCompactionMetadata, createPassthroughCompactionMetadata } from "./compaction-metadata.js";
...
export function sliceTextChars(text, start, end) {
...
export function countTerminalCells(text) {
...
export function trimEmptyEdges(lines) {
    let start = 0;
...
export function dedupeAdjacent(lines) {
    const next = [];
...
export function headTail(lines, head, tail, noOmit = false) {
...
export function clampTextWithMetadata(text, maxChars, noOmit = false) {
    if (countTextChars(text) <= maxChars) {
        return { text };
    }
    if (noOmit) {
...
export function clampTextMiddleWithMetadata(text, maxChars, noOmit = false) {
    if (countTextChars(text) <= maxChars) {
        return { text };
    }
    if (noOmit) {
