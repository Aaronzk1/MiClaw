...
import { collectMarkerDelimitedBlockIssues, inspectMarkerDelimitedBlock, uninstallMarkerDelimitedBlock, } from "../shared/marker-instructions.js";
...
const TOKENJUICE_ONA_ADVISORY = "Ona support is beta and skill-based; Ona discovers .ona/skills/tokenjuice/SKILL.md in workspace sessions, but tokenjuice does not intercept shell output.";
...
function hasMalformedLegacyMarkerStructure(text, completeBlockCount) {
...
async function resolveCanonicalProjectDir(options = {}) {
...
async function resolveCanonicalInstructionPath(filePath) {
...
async function rejectSkillSymlink(filePath) {
    try {
...
async function rejectSkillPathComponentSymlinks(filePath, projectDir) {
...
function getExpectedSkillPath(projectDir) {
    return join(projectDir, TOKENJUICE_ONA_SKILL_PATH);
}
async function resolveSafeProjectSkillPath(filePath, projectDir, realProjectDir = projectDir) {
    const resolvedPath = resolve(filePath);
    await rejectSkillSymlink(projectDir);
...
async function getDefaultSkillPath(options = {}) {
...
async function getDefaultLegacyInstructionsPath(options = {}) {
...
async function skillArtifactExists(filePath) {
    try {
...
async function resolveSkillPath(skillPath, options = {}) {
    if (skillPath) {
        const projectDir = await resolveCanonicalProjectDir(options);
...
function hasHexDigits(value, start, length) {
    if (start + length > value.length) {
        return false;
    }
    return /^[0-9A-Fa-f]+$/u.test(value.slice(start, start + length));
}
function isValidDoubleQuotedYamlScalar(value) {
...
function isValidSimpleYamlScalar(rawValue) {
...
function inspectSkillFrontmatter(text) {
...
const TOKENJUICE_ONA_SKILL = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When Ona Agent runs terminal commands likely to produce long output, prefer `tokenjuice wrap -- <command>`.",
    }),
...
async function inspectLegacyOnaInstructions(options = {}) {
...
async function uninstallLegacyOnaInstructions(options = {}) {
    const instructionsPath = await getDefaultLegacyInstructionsPath(options);
    if (await isSymlink(instructionsPath)) {
    ...
    if (legacy.unsafeIssues.length > 0) {
        throw new Error(`cannot safely migrate malformed tokenjuice markers in ${legacy.instructionsPath}; remove the dangling marker manually, then rerun tokenjuice install ona`);
    }
...
export async function installOnaSkill(skillPath, options = {}) {
    ...
    if (legacyBeforeInstall.unsafeIssues.length > 0) {
        throw new Error(`cannot safely migrate malformed tokenjuice markers in ${legacyBeforeInstall.instructionsPath}; remove the dangling marker manually, then rerun tokenjuice install ona`);
    }
...
export async function uninstallOnaSkill(skillPath, options = {}) {
    const resolvedSkillPath = await resolveSkillPath(skillPath, options);
    const legacyBeforeUninstall = await inspectLegacyOnaInstructions(options);
    if (legacyBeforeUninstall.unsafeIssues.length > 0) {
        throw new Error(`cannot safely migrate malformed tokenjuice markers in ${legacyBeforeUninstall.instructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall ona`);
    }
...
export async function doctorOnaSkill(skillPath, options = {}) {
    ...
    if (!existing.exists) {
        ...
        if (legacy.hasLegacyBlock) {
            const issues = legacy.issues.length > 0
                ? legacy.issues
                : [
    ...
    const legacy = await inspectLegacyOnaInstructions(options);
    const legacyIssues = legacy.hasLegacyBlock && legacy.issues.length === 0
...
export async function installOnaInstructions(instructionsPath, options = {}) {
...
export async function uninstallOnaInstructions(instructionsPath, options = {}) {
    const resolvedOptions = instructionsPath ? { ...options, projectDir: dirname(resolve(instructionsPath)) } : options;
    const result = await uninstallOnaSkill(undefined, resolvedOptions);
    return { ...result, instructionsPath: result.skillPath };
}
export async function doctorOnaInstructions(instructionsPath, options = {}) {
    const resolvedOptions = instructionsPath ? { ...options, projectDir: dirname(resolve(instructionsPath)) } : options;
