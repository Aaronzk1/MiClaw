...
const TOKENJUICE_STAGEWISE_ADVISORY = "Stagewise support is beta and skill-based; Stagewise discovers .stagewise/skills/tokenjuice/SKILL.md in workspace sessions, but tokenjuice does not intercept shell output.";
...
async function resolveSafeProjectSkillPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use Stagewise skill ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
    ...
    if (resolvedPath !== expectedSkillPath) {
        throw new Error(`cannot use Stagewise skill ${resolvedPath}; tokenjuice only installs the project-local .stagewise/skills/tokenjuice/SKILL.md file`);
    }
    return resolvedPath;
}
...
const TOKENJUICE_STAGEWISE_SKILL = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When Stagewise agent runs terminal commands likely to produce long output, prefer `tokenjuice wrap -- <command>`.",
    }),
...
export async function installStagewiseSkill(skillPath, options = {}) {
...
export async function uninstallStagewiseSkill(skillPath, options = {}) {
    const resolvedSkillPath = await resolveSkillPath(skillPath, options);
...
export async function doctorStagewiseSkill(skillPath, options = {}) {
