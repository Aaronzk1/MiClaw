...
const TOKENJUICE_CLAWDBOT_ADVISORY = "Clawdbot support is beta and skill-based; Clawdbot discovers skills/tokenjuice/SKILL.md in workspace sessions, but tokenjuice does not intercept shell output.";
...
async function resolveSafeProjectSkillPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use Clawdbot skill ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
    ...
    if (resolvedPath !== expectedSkillPath) {
        throw new Error(`cannot use Clawdbot skill ${resolvedPath}; tokenjuice only installs the project-local skills/tokenjuice/SKILL.md file`);
    }
    return resolvedPath;
}
...
const TOKENJUICE_CLAWDBOT_SKILL = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When Clawdbot agent runs terminal commands likely to produce long output, prefer `tokenjuice wrap -- <command>`.",
    }),
...
export async function installClawdbotSkill(skillPath, options = {}) {
...
export async function uninstallClawdbotSkill(skillPath, options = {}) {
    const resolvedSkillPath = await resolveSkillPath(skillPath, options);
...
export async function doctorClawdbotSkill(skillPath, options = {}) {
