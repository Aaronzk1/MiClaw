...
async function findParentTokenjuiceInstructionPaths(startDir, boundaryDir) {
    let current = resolve(startDir);
...
async function findDescendantTokenjuiceInstructionPaths(projectDir) {
    const paths = [];
...
async function getDefaultInstructionPaths(options = {}, preferExistingTokenjuiceBlocks = false, includeRootWithExistingBlocks = false) {
...
export async function installOpenInterpreterInstructions(instructionsPath, options = {}) {
    ...
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
        ...
        if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
            throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install open-interpreter`);
        }
    }
    const results = [];
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
...
export async function uninstallOpenInterpreterInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPaths = instructionsPath ? [instructionsPath] : await getDefaultUninstallInstructionPaths(options);
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
        const existing = await readInstructionFile(resolvedInstructionsPath);
        const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_OPEN_INTERPRETER_BLOCK_CONFIG);
        if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
            throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall open-interpreter`);
        }
    }
    const results = [];
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
...
async function inspectOpenInterpreterInstructionsPath(resolvedInstructionsPath) {
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_OPEN_INTERPRETER_BLOCK_CONFIG);
...
export async function doctorOpenInterpreterInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPaths = instructionsPath ? [instructionsPath] : await getDefaultInstructionPaths(options, true, true);
    const reports = [];
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
