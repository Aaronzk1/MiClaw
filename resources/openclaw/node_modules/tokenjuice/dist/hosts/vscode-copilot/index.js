...
import { ensureHooksDir, findStrayTokenjuiceHookFiles, isRecord, loadCopilotHooksConfigWithBackup, readCopilotHooksConfig, sanitizeCopilotHooksConfig, writeCopilotHooksConfigAtomic, } from "../shared/hooks-json-file.js";
...
const TOKENJUICE_VSCODE_COPILOT_MATCHER = `${TOKENJUICE_VSCODE_COPILOT_TOOL_NAME}|${TOKENJUICE_VSCODE_COPILOT_LEGACY_TOOL_NAME}`;
...
const TOKENJUICE_VSCODE_COPILOT_ADVISORY = "enable `chat.useHooks` in VS Code settings and trust the workspace; both are required for the hook to fire.";
const TOKENJUICE_VSCODE_COPILOT_INSTRUCTIONS_ADVISORY = "paste the snippet from `tokenjuice doctor vscode-copilot --print-instructions` into `.github/copilot-instructions.md` or `AGENTS.md` so the agent respects compacted output and knows when to prefix a command with `tokenjuice wrap --raw --`.";
...
`;
export function getVscodeCopilotInstructionsSnippet() {
...
function getVscodeCopilotHooksDir() {
    return join(homedir(), ".copilot", "hooks");
}
function getDefaultHooksPath() {
...
async function buildVscodeCopilotHookCommand(options = {}) {
...
function getPreToolUseHooks(config) {
...
function findTokenjuiceVscodeCopilotHookCommand(config) {
...
async function migrateLegacyInstall(hooksDir, hooksPath) {
...
export async function installVscodeCopilotHook(hooksPath = getDefaultHooksPath(), options = {}) {
...
export async function uninstallVscodeCopilotHook(hooksPath = getDefaultHooksPath()) {
    ...
    const retainedCurrent = currentPreToolUse.filter((hook) => !isTokenjuiceVscodeCopilotHook(hook));
    const retainedLegacy = legacyPreToolUse.filter((hook) => !isTokenjuiceVscodeCopilotHook(hook));
    const removed = currentPreToolUse.length + legacyPreToolUse.length - retainedCurrent.length - retainedLegacy.length;
    if (removed === 0) {
        return { hooksPath, removed: 0, deletedFile: false };
    }
    ...
    const fileIsEmpty = Object.keys(config.hooks).length === 0
...
async function findStrayVscodeCopilotHookFiles(hooksDir, canonicalPath) {
    return findStrayTokenjuiceHookFiles(hooksDir, canonicalPath, (command) => command.includes(TOKENJUICE_VSCODE_COPILOT_SUBCOMMAND));
}
export async function doctorVscodeCopilotHook(hooksPath = getDefaultHooksPath(), options = {}) {
    ...
    if (missingPaths.length > 0) {
        issues.push(`configured vscode-copilot hook points at missing path${missingPaths.length === 1 ? "" : "s"}`);
    }
...
function commandAlreadyWrapped(command) {
    const argv = parseShellWords(stripLeadingCdPrefix(command));
    if (argv.length < 2) {
        return false;
    }
...
function readPayloadField(payload, snakeKey, camelKey) {
    return payload[snakeKey] ?? payload[camelKey];
}
function isTerminalToolName(value) {
    return value === TOKENJUICE_VSCODE_COPILOT_TOOL_NAME || value === TOKENJUICE_VSCODE_COPILOT_LEGACY_TOOL_NAME;
}
export async function runVscodeCopilotPreToolUseHook(rawText, wrapLauncher = "tokenjuice") {
    let payload;
    try {
    ...
    const wrappedCommand = `${launcherCommand} wrap --source ${TOKENJUICE_VSCODE_COPILOT_SOURCE} -- ${shellQuote(shell.path)} ${shell.flag} ${shellQuote(command)}`;
    const updatedInput = {
        ...toolInput,
