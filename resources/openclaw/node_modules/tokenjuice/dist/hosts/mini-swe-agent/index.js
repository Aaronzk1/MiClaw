...
const TOKENJUICE_MINI_SWE_AGENT_ADVISORY = "mini-SWE-agent support is beta and config-fragment based; load it with mini -c mini.yaml -c .mini-swe-agent/tokenjuice.yaml.";
function isTokenjuiceMiniSweAgentConfigText(text) {
...
async function chooseMiniSweAgentBackupPath(configPath) {
...
async function writeMiniSweAgentConfigWithoutBackup(configPath, text) {
...
function buildMiniSweAgentConfig({ restoreBackupSuffix } = {}) {
    return [
        ...
        ...buildTokenjuiceGuidanceBullets({
            wrapBullet: `- For mini-SWE-agent bash commands likely to produce long output, run them through \`${TOKENJUICE_WRAP_COMMAND}\`.`,
        }).map((bullet) => `# ${bullet}`),
        "",
    ].join("\n");
}
export async function installMiniSweAgentConfig(configPath, options = {}) {
    ...
    if (existing.exists && isTokenjuiceMiniSweAgentConfigText(existing.text)) {
        ...
        const result = await writeInstructionFile(resolvedConfigPath, buildMiniSweAgentConfig({ restoreBackupSuffix }));
        return {
    ...
    if (existing.exists) {
        ...
        await writeMiniSweAgentConfigWithoutBackup(resolvedConfigPath, buildMiniSweAgentConfig({ restoreBackupSuffix: backupPath.slice(resolvedConfigPath.length) }));
...
export async function uninstallMiniSweAgentConfig(configPath, options = {}) {
    const resolvedConfigPath = configPath ?? (await getDefaultConfigPath(options));
    const existing = await readInstructionFile(resolvedConfigPath);
    if (!existing.exists || !isTokenjuiceMiniSweAgentConfigText(existing.text)) {
...
export async function doctorMiniSweAgentConfig(configPath, options = {}) {
    const resolvedConfigPath = configPath ?? (await getDefaultConfigPath(options));
    const existing = await readInstructionFile(resolvedConfigPath);
    if (!existing.exists) {
        return {
