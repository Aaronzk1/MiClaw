...
const TOKENJUICE_TABBY_ADVISORY = "Tabby support is beta and system-prompt based; it guides answer behavior through ~/.tabby/config.toml but does not intercept tool output.";
...
function getTabbyConfigDir(options = {}) {
...
async function findTabbyConfigSymlink(configPath, includeInstallSidecars = false) {
    const candidates = [
...
async function assertNoTabbyConfigSymlink(configPath, operation, includeInstallSidecars = false) {
...
async function getExistingFileMode(filePath, existing) {
    if (!existing.exists) {
        return undefined;
    }
    return (await lstat(filePath)).mode & 0o777;
}
async function writeTabbyConfigAtomically(filePath, text, mode) {
...
async function chooseTabbyBackupPath(filePath) {
...
async function writeTabbyConfigFile(filePath, text, existing) {
...
function hasUnsafeMarkerStructure(text) {
    const begin = getStructuralMarkerLines(text, TOKENJUICE_TABBY_BEGIN);
    const end = getStructuralMarkerLines(text, TOKENJUICE_TABBY_END);
...
function findTomlMultilineClose(line, delimiter, fromIndex) {
...
function updateTomlMultilineState(line, state) {
    let index = 0;
...
function getTomlStructureLines(lines) {
    const state = { delimiter: undefined };
...
function findAnswerTableRange(lines) {
    const structureLines = getTomlStructureLines(lines);
...
function hasUserOwnedSystemPrompt(text) {
    const lines = text.split("\n");
    const structureLines = getTomlStructureLines(lines);
...
function createSystemPromptBlock(mode) {
    return [
...
function installTokenjuiceBlock(text) {
...
export async function installTabbySystemPrompt(configPath, options = {}) {
    ...
    if (existing.exists && hasUnsafeMarkerStructure(existing.text)) {
        throw new Error(`cannot safely repair malformed tokenjuice Tabby markers in ${resolvedConfigPath}; remove the dangling marker manually, then rerun tokenjuice install tabby`);
    }
    const withoutTokenjuice = removeTokenjuiceBlock(existing.text).text;
    if (hasInlineAnswerTable(withoutTokenjuice)) {
        throw new Error(`cannot install Tabby guidance because ${resolvedConfigPath} defines answer as an inline TOML table; convert it to an [answer] table or root answer.* dotted keys, then rerun tokenjuice install tabby`);
    }
    if (hasRootAnswerAssignment(withoutTokenjuice)) {
        throw new Error(`cannot install Tabby guidance because ${resolvedConfigPath} defines answer as a root TOML value; convert it to an [answer] table or root answer.* dotted keys, then rerun tokenjuice install tabby`);
    }
    if (hasAnswerArrayTable(withoutTokenjuice)) {
        throw new Error(`cannot install Tabby guidance because ${resolvedConfigPath} defines answer as an array of TOML tables; convert it to one [answer] table or root answer.* dotted keys, then rerun tokenjuice install tabby`);
    }
    if (hasUserOwnedSystemPrompt(withoutTokenjuice)) {
        throw new Error(`cannot install Tabby guidance because ${resolvedConfigPath} already defines [answer].system_prompt; merge tokenjuice guidance manually or remove that setting, then rerun tokenjuice install tabby`);
    }
    return writeTabbyConfigFile(resolvedConfigPath, installTokenjuiceBlock(existing.text), existing);
}
export async function uninstallTabbySystemPrompt(configPath, options = {}) {
    const resolvedConfigPath = configPath ?? getDefaultConfigPath(options);
    ...
    if (hasUnsafeMarkerStructure(existing.text)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice Tabby markers in ${resolvedConfigPath}; remove the dangling marker manually, then rerun tokenjuice uninstall tabby`);
    }
...
export async function doctorTabbySystemPrompt(configPath, options = {}) {
    const resolvedConfigPath = configPath ?? getDefaultConfigPath(options);
