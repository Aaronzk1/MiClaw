...
const TOKENJUICE_AICTL_ADVISORY = "aictl support is beta and prompt-file based; aictl appends AICTL.md project instructions to the system prompt, but tokenjuice does not intercept shell output.";
...
function getPromptFileNameForDisplay() {
    try {
...
async function resolveSafeProjectInstructionsPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use aictl instructions ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
...
const TOKENJUICE_AICTL_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When aictl runs terminal commands likely to produce long output through `exec_shell`, prefer `tokenjuice wrap -- <command>`.",
    }),
...
export async function installAictlInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install aictl`);
    }
...
export async function uninstallAictlInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = await resolveInstructionsPath(instructionsPath, options);
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_AICTL_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall aictl`);
    }
...
export async function doctorAictlInstructions(instructionsPath, options = {}) {
    ...
    try {
        ...
        return {
            ...
            ...buildInstructionDoctorReportFields({
                ...
                fixCommand: error.message.includes("outside")
    ...
    return {
        ...
        ...buildInstructionDoctorReportFields({
            ...
            issues,
            advisory: TOKENJUICE_AICTL_ADVISORY,
            fixCommand: markerIssues.length > 0 || structureIssues.length > 0
