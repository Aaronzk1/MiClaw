...
const TOKENJUICE_NOVAKIT_ADVISORY = "NovaKit support is beta and instruction-based; NovaKit loads NOVAKIT.md context files, but tokenjuice does not intercept tool output.";
...
async function resolveSafeProjectInstructionsPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use NovaKit instructions ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
...
const TOKENJUICE_NOVAKIT_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When running terminal commands through NovaKit, prefer `tokenjuice wrap -- <command>` for commands likely to produce long output.",
    }),
...
export async function installNovaKitInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install novakit`);
    }
...
export async function uninstallNovaKitInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = await resolveInstructionsPath(instructionsPath, options);
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_NOVAKIT_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall novakit`);
    }
...
export async function doctorNovaKitInstructions(instructionsPath, options = {}) {
