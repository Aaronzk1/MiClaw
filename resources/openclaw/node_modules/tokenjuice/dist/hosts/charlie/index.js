...
const TOKENJUICE_CHARLIE_ADVISORY = "Charlie support is beta and AGENTS.md-based; Charlie still owns planning, coding, review, and command execution.";
...
async function rejectInstructionSymlink(filePath) {
    try {
...
function getExpectedInstructionsPath(projectDir) {
    return join(projectDir, "AGENTS.md");
}
async function resolveSafeProjectInstructionsPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use Charlie instructions ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
    ...
    if (resolvedPath !== expectedInstructionsPath) {
        throw new Error(`cannot use Charlie instructions ${resolvedPath}; tokenjuice only installs the project-local AGENTS.md file`);
    }
    return resolvedPath;
}
...
async function instructionsArtifactExists(filePath) {
    try {
...
async function resolveInstructionsPath(instructionsPath, options = {}) {
    if (instructionsPath) {
        const projectDir = await resolveProjectDir(options);
        const realProjectDir = await realpath(projectDir).catch(() => projectDir);
...
const TOKENJUICE_CHARLIE_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When running terminal commands through Charlie, prefer `tokenjuice wrap -- <command>` for commands likely to produce long output.",
    }),
...
export async function installCharlieInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install charlie`);
    }
...
export async function uninstallCharlieInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = await resolveInstructionsPath(instructionsPath, options);
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_CHARLIE_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall charlie`);
    }
...
export async function doctorCharlieInstructions(instructionsPath, options = {}) {
    let resolvedInstructionsPath;
    try {
