...
const TOKENJUICE_ROO_ADVISORY = "Roo Code support is beta and rule-based; it guides command usage but does not intercept tool output.";
...
const TOKENJUICE_ROO_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- For Roo `execute_command` terminal commands likely to produce long output, run them through `tokenjuice wrap -- <command>`.",
    }),
    TOKENJUICE_ROO_END,
].join("\n");
...
export async function installRooInstructions(instructionsPath, options = {}) {
...
export async function uninstallRooInstructions(instructionsPath = getDefaultInstructionsPath()) {
...
export async function doctorRooInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = instructionsPath ?? getDefaultInstructionsPath(options);
