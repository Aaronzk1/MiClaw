...
import { collectMarkerDelimitedBlockIssues, inspectMarkerDelimitedBlock, removeMarkerDelimitedBlock, upsertMarkerDelimitedBlock, } from "../shared/marker-instructions.js";
...
const TOKENJUICE_DOT_AGENTS_ADVISORY = "dot-agents support is beta and rule-based; run `dot-agents sync` after install or uninstall so dot-agents propagates global rules to managed agent configs.";
function getDotAgentsConfigDir(options = {}) {
    return resolve(options.configDir || process.env.DOT_AGENTS_HOME || join(homedir(), ".agents"));
}
function getDefaultRulePath(options = {}) {
...
const TOKENJUICE_DOT_AGENTS_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When dot-agents propagates this global rule into managed coding-agent configs, prefer `tokenjuice wrap -- <command>` for terminal commands likely to produce long output.",
    }),
...
function ensureDotAgentsFrontmatter(text) {
...
export async function installDotAgentsRule(rulePath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedRulePath}; remove the dangling marker manually, then rerun tokenjuice install dot-agents`);
    }
...
export async function uninstallDotAgentsRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? getDefaultRulePath(options);
    const existing = await readInstructionFile(resolvedRulePath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_DOT_AGENTS_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedRulePath}; remove the dangling marker manually, then rerun tokenjuice uninstall dot-agents`);
    }
...
export async function doctorDotAgentsRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? getDefaultRulePath(options);
    const existing = await readInstructionFile(resolvedRulePath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_DOT_AGENTS_BLOCK_CONFIG);
    ...
    const malformedMarkerIssues = hasMalformedMarkers && markerIssues.length === 0
        ? [
