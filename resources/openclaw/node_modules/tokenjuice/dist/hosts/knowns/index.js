...
const TOKENJUICE_KNOWNS_ADVISORY = "Knowns support is beta and instruction-based; Knowns generates KNOWNS.md guidance and exposes memory, tasks, docs, and code intelligence through MCP, but tokenjuice does not intercept tool output.";
...
async function resolveSafeProjectInstructionsPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use Knowns instructions ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
...
const TOKENJUICE_KNOWNS_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When an AI assistant working from Knowns context runs terminal commands, prefer `tokenjuice wrap -- <command>` for commands likely to produce long output.",
    }),
...
export async function installKnownsInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install knowns`);
    }
...
export async function uninstallKnownsInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = await resolveInstructionsPath(instructionsPath, options);
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_KNOWNS_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall knowns`);
    }
...
export async function doctorKnownsInstructions(instructionsPath, options = {}) {
