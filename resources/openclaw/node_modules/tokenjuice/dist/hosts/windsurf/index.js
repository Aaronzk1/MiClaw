...
const TOKENJUICE_WINDSURF_ADVISORY = "Windsurf support is beta and rule-based; it guides command usage but does not intercept tool output.";
...
function hasAlwaysOnFrontmatter(text) {
...
export async function installWindsurfRule(rulePath, options = {}) {
...
export async function uninstallWindsurfRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? getDefaultRulePath(options);
    const result = await removeInstructionFile(resolvedRulePath);
    return { rulePath: result.filePath, removed: result.removed };
}
export async function doctorWindsurfRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? getDefaultRulePath(options);
