...
const TOKENJUICE_AI_MEMORY_PROTOCOL_ADVISORY = "AI Memory Protocol support is beta and RST-memory based; run memory init before install; tokenjuice records command guidance in the memory graph but does not intercept MCP tool output.";
...
function getDefaultMemoryPath(options = {}) {
    return join(getMemoryDir(options), "memory", "preferences.rst");
}
function resolveMemoryLocation(memoryPath, options = {}) {
...
async function readOptionalMemoryFile(filePath) {
    try {
        return await readInstructionFile(filePath);
    }
    catch (error) {
        if (["EISDIR", "ENOTDIR"].includes(error.code ?? "")) {
...
async function findMemoryPathSymlink(location) {
    const candidates = [
...
async function assertNoMemoryPathSymlink(location, operation) {
...
async function collectMissingWorkspacePaths(location) {
    const checks = await Promise.all(location.workspacePaths.map(async (filePath) => [filePath, await pathExists(filePath)]));
    return checks.filter(([, exists]) => !exists).map(([filePath]) => filePath);
}
function memoryIndexSupportsPreferences(text) {
...
async function collectWorkspaceIssues(location) {
...
async function assertInitializedWorkspace(location) {
    const workspaceCheck = await collectWorkspaceIssues(location);
    if (workspaceCheck.issues.length === 0) {
        return;
    }
    throw new Error(`AI Memory Protocol workspace is not initialized or incompatible at ${location.memoryDir}; run ${buildMemoryInitCommand(location.memoryDir)} before ${buildTargetedInstallCommand(location.memoryDir)} (${workspaceCheck.issues.join("; ")})`);
}
...
function buildAiMemoryProtocolBlock(now = new Date()) {
    ...
    return [
        ...
        ...buildTokenjuiceGuidanceBullets({
            wrapBullet: "- When an AI coding agent using AI Memory Protocol needs to run a terminal command likely to produce long output, prefer `tokenjuice wrap -- <command>`.",
        }).map((line) => ` ${line}`),
...
function buildMemoryInitCommand(memoryDir) {
    return `memory init ${shellQuote(memoryDir)} --install`;
}
function buildTargetedInstallCommand(memoryDir) {
...
function ensurePreferencesHeader(text) {
    if (text.trim()) {
        return text;
    }
    return "===========\nPreferences\n===========\n\n";
}
export async function installAiMemoryProtocolMemory(memoryPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${location.memoryPath}; remove the dangling marker manually, then rerun tokenjuice install ai-memory-protocol`);
    }
    if (!existing.exists) {
        ...
        const result = await writeInstructionFile(location.memoryPath, `${ensurePreferencesHeader(existing.text)}${buildAiMemoryProtocolBlock()}\n`);
...
export async function uninstallAiMemoryProtocolMemory(memoryPath, options = {}) {
    const location = resolveMemoryLocation(memoryPath, options);
    ...
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_AI_MEMORY_PROTOCOL_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${location.memoryPath}; remove the dangling marker manually, then rerun tokenjuice uninstall ai-memory-protocol`);
    }
...
export async function doctorAiMemoryProtocolMemory(memoryPath, options = {}) {
    const location = resolveMemoryLocation(memoryPath, options);
    const symlink = await findMemoryPathSymlink(location);
    if (symlink) {
        return {
