...
const TOKENJUICE_LEANCTL_ADVISORY = "LeanCTL support is beta and instruction-based; LeanCTL owns command execution, so tokenjuice writes guidance to .leanctl/instructions.md instead of intercepting tool output.";
function isTokenjuiceLeanCtlInstructionsText(text) {
...
async function chooseLeanCtlBackupPath(instructionsPath) {
...
async function resolveSafeProjectInstructionsPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use LeanCTL instructions ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
...
function buildLeanCtlInstructions({ restoreBackupSuffix } = {}) {
    return [
        ...
        ...buildTokenjuiceGuidanceBullets({
            wrapBullet: "- When running terminal commands through LeanCTL, prefer `tokenjuice wrap -- <command>` for commands likely to produce long output.",
        }),
        "- LeanCTL loads this file from `.leanctl/instructions.md` as project instructions.",
        "",
    ].join("\n");
}
export async function installLeanCtlInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists) {
        ...
        await writeTextFileWithoutBackup(resolvedInstructionsPath, buildLeanCtlInstructions({ restoreBackupSuffix: backupPath.slice(resolvedInstructionsPath.length) }));
...
export async function uninstallLeanCtlInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = await resolveInstructionsPath(instructionsPath, options);
    const existing = await readInstructionFile(resolvedInstructionsPath);
    if (!existing.exists || !isTokenjuiceLeanCtlInstructionsText(existing.text)) {
...
export async function doctorLeanCtlInstructions(instructionsPath, options = {}) {
