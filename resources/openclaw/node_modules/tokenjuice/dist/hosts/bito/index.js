...
import { inspectMarkerDelimitedBlock, installMarkerDelimitedBlock, uninstallMarkerDelimitedBlock, } from "../shared/marker-instructions.js";
...
import { assertSafeInstructionBackupPath, collectGuidanceIssues, readInstructionFile, removeInstructionFile, writeInstructionFile, } from "../shared/instruction-file.js";
...
const TOKENJUICE_BITO_ADVISORY = "Bito support is beta and custom-guidelines based; Bito still owns PR review, analysis settings, tools, and comment delivery.";
...
async function rejectInstructionSymlink(filePath, label) {
    try {
...
async function rejectSymlinkPathComponents(filePath, projectDir, label) {
...
async function resolveSafeProjectPath(filePath, projectDir, realProjectDir, label) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use Bito ${label} ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
    await rejectInstructionSymlink(resolvedPath, label);
    return resolvedPath;
}
async function getProjectPaths(options = {}) {
...
async function getProjectAliasPaths(options = {}) {
    const projectDir = await resolveProjectDir(options);
    return {
...
async function resolveSafeProjectPathIfAvailable(filePath, projectDir, realProjectDir, label) {
    try {
...
async function detectTokenjuiceMarkerEvidence(options = {}) {
    const projectDir = await resolveProjectDir(options);
    const realProjectDir = await realpath(projectDir).catch(() => projectDir);
    ...
    const guidelinesPath = await resolveSafeProjectPathIfAvailable(join(projectDir, ".bito", "tokenjuice.md"), projectDir, realProjectDir, "guidelines");
    if (guidelinesPath) {
...
const TOKENJUICE_BITO_GUIDELINES = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When Bito review, chat, or tool workflows suggest terminal commands likely to produce long output, prefer `tokenjuice wrap -- <command>`.",
    }),
...
function getTokenjuiceConfigBlockText(text) {
...
function removeTokenjuiceConfigBlockText(text) {
    const beginIndex = text.indexOf(TOKENJUICE_BITO_BEGIN);
    if (beginIndex === -1) {
        return text;
    }
...
export async function installBitoGuidelines(options = {}) {
    ...
    if (existingConfig.exists && hasMalformedMarkerStructure(existingConfig.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice Bito markers in ${paths.configPath}; remove the dangling marker manually, then rerun tokenjuice install bito`);
    }
    if (existingConfig.exists && hasUserOwnedCustomGuidelines(existingConfig.text)) {
        throw new Error(`cannot install Bito guidance because ${paths.configPath} already defines custom_guidelines; add ${TOKENJUICE_BITO_GUIDELINES_RELATIVE_PATH} manually or remove the custom_guidelines block, then rerun tokenjuice install bito`);
    }
...
export async function uninstallBitoGuidelines(options = {}) {
    const paths = await getProjectPaths(options);
    const existingConfig = await readInstructionFile(paths.configPath);
    const markerState = inspectMarkerDelimitedBlock(existingConfig.text, TOKENJUICE_BITO_CONFIG);
    if (existingConfig.exists && hasMalformedMarkerStructure(existingConfig.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice Bito markers in ${paths.configPath}; remove the dangling marker manually, then rerun tokenjuice uninstall bito`);
    }
...
export async function doctorBitoGuidelines(options = {}) {
    let paths;
    try {
