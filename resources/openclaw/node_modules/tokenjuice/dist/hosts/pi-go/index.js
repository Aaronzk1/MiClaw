...
const TOKENJUICE_PI_GO_ADVISORY = "pi-go support is beta and skill-based; pi-go discovers .pi/skills/tokenjuice/SKILL.md when loading project skills, but tokenjuice does not intercept shell output.";
function isTokenjuicePiGoSkillText(text) {
...
async function choosePiGoBackupPath(skillPath) {
...
async function resolveSafeProjectSkillPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use pi-go skill ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
...
function buildPiGoSkill({ restoreBackupSuffix } = {}) {
    return [
        ...
        ...buildTokenjuiceGuidanceBullets({
            wrapBullet: "- When pi-go runs terminal commands likely to produce long output, prefer `tokenjuice wrap -- <command>`.",
        }),
...
export async function installPiGoSkill(skillPath, options = {}) {
    ...
    if (existing.exists) {
        ...
        await writeTextFileWithoutBackup(resolvedSkillPath, buildPiGoSkill({ restoreBackupSuffix: backupPath.slice(resolvedSkillPath.length) }));
        return { skillPath: resolvedSkillPath, backupPath };
    }
    await writeTextFileWithoutBackup(resolvedSkillPath, buildPiGoSkill());
    return { skillPath: resolvedSkillPath };
}
export async function uninstallPiGoSkill(skillPath, options = {}) {
    const resolvedSkillPath = await resolveSkillPath(skillPath, options);
    const existing = await readInstructionFile(resolvedSkillPath);
    if (!existing.exists || !isTokenjuicePiGoSkillText(existing.text)) {
...
export async function doctorPiGoSkill(skillPath, options = {}) {
