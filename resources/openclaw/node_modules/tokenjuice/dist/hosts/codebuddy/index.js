...
function isNativeWindowsCodeBuddyUnsupported() {
...
async function buildCodeBuddyHookCommand(options = {}) {
...
function getCodeBuddyFixCommand(local = false) {
    return local ? "tokenjuice install codebuddy --local" : TOKENJUICE_CODEBUDDY_FIX_COMMAND;
}
function createTokenjuiceCodeBuddyHook(command) {
    return {
        matcher: "Bash",
        hooks: [
            {
...
function isTokenjuiceCodeBuddyHookEntry(hook) {
    if (!isRecord(hook)) {
        return false;
    }
    ...
    return (hook.command.includes(TOKENJUICE_CODEBUDDY_HOOK_SUBCOMMAND)
...
function isTokenjuiceCodeBuddyHook(group) {
    return group.hooks.some(isTokenjuiceCodeBuddyHookEntry);
}
function findTokenjuiceCodeBuddyHookCommand(config) {
    const preToolUse = Array.isArray(config.hooks.PreToolUse) ? config.hooks.PreToolUse : [];
    for (const group of preToolUse) {
        if (!(isRecord(group) && Array.isArray(group.hooks) && isTokenjuiceCodeBuddyHook(group))) {
            continue;
        }
        const command = group.hooks.find((hook) => isRecord(hook)
            && (hook.statusMessage === TOKENJUICE_CODEBUDDY_STATUS
                || (typeof hook.command === "string" && hook.command.includes(TOKENJUICE_CODEBUDDY_HOOK_SUBCOMMAND))))?.command;
...
function sanitizeCodeBuddySettings(raw) {
    if (!isRecord(raw)) {
        return { hooks: {} };
    }
    return {
...
async function loadCodeBuddySettings(settingsPath) {
    try {
...
async function readCodeBuddySettings(settingsPath) {
    try {
        const rawText = await readFile(settingsPath, "utf8");
        const parsed = JSON.parse(rawText);
        return {
...
async function resolveCodeBuddyHostShell(toolInput) {
...
export async function installCodeBuddyHook(settingsPath = getDefaultSettingsPath(), options = {}) {
...
export async function uninstallCodeBuddyHook(settingsPath = getDefaultSettingsPath()) {
    let rawText;
    try {
...
export async function doctorCodeBuddyHook(settingsPath = getDefaultSettingsPath(), options = {}) {
    if (isNativeWindowsCodeBuddyUnsupported()) {
...
export async function runCodeBuddyPreToolUseHook(rawText, wrapLauncher = "tokenjuice") {
    let payload;
    try {
