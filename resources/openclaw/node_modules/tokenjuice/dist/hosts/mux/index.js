...
function getMuxFixCommand(local) {
    return local ? `${TOKENJUICE_MUX_FIX_COMMAND} --local` : TOKENJUICE_MUX_FIX_COMMAND;
}
async function findMuxHookSymlink(hookPath) {
...
async function assertNoMuxHookSymlink(hookPath, operation) {
...
async function assertNoMuxBackupSymlink(backupPath) {
    try {
...
async function chooseMuxBackupPath(hookPath) {
...
async function getDefaultHookPath(options = {}) {
    return join(await resolveProjectDir(options), ".mux", "tool_post");
}
function buildHookBlock(command) {
    return [
...
function hasBashShebang(script) {
...
function hasUnsupportedExistingHook(script) {
    return Boolean(script.trim()) && !hasBashShebang(script);
}
function executableModeForExistingHook(mode) {
    const permissions = mode & 0o777;
    return permissions | ((permissions & 0o444) >> 2) | 0o100;
}
function backupModeForExistingHook(mode) {
...
function removeTokenjuiceBlock(script) {
...
function upsertTokenjuiceBlock(existing, command) {
...
async function writeMuxHookFile(hookPath, command) {
    ...
    try {
        ...
        if (error.code !== "ENOENT" || existingExists) {
            throw error;
        }
    }
...
export async function installMuxHook(hookPath, options = {}) {
...
export async function uninstallMuxHook(hookPath, options = {}) {
    const resolvedHookPath = hookPath ?? (await getDefaultHookPath(options));
...
export async function doctorMuxHook(hookPath, options = {}) {
    const resolvedHookPath = hookPath ?? (await getDefaultHookPath(options));
...
function readCommand(env, input) {
...
function readExitCode(env, result) {
...
export async function runMuxPostToolUseHook(env = process.env) {
