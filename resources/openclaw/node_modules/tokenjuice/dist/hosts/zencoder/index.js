...
function getLeadingFrontmatterLines(text) {
...
function hasAlwaysApplyFrontmatter(text) {
    return getLeadingFrontmatterLines(text).some((line) => line.trim() === "alwaysApply: true");
}
function hasDescriptionFrontmatter(text) {
...
function buildZencoderRule({ restoreBackupSuffix } = {}) {
    return [
        "---",
...
function isTokenjuiceZencoderRuleText(text) {
...
async function chooseBackupSuffix(rulePath) {
...
async function writeZencoderRuleWithoutBackup(rulePath, { restoreBackupSuffix } = {}) {
...
export async function installZencoderRule(rulePath, options = {}) {
...
export async function uninstallZencoderRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
    const existing = await readInstructionFile(resolvedRulePath);
    if (existing.exists && !isTokenjuiceZencoderRuleText(existing.text)) {
        throw new Error(`refusing to remove ${resolvedRulePath}; it does not look like the tokenjuice Zencoder rule. Review and remove it manually, or reinstall tokenjuice zencoder first.`);
    }
    const restoreBackupSuffix = readRestoreBackupSuffix(existing.text);
...
export async function doctorZencoderRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
    const existing = await readInstructionFile(resolvedRulePath);
    if (!existing.exists) {
        return {
