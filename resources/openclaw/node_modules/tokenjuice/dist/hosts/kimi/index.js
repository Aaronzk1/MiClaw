...
const TOKENJUICE_KIMI_ADVISORY = "Kimi support is beta and injects compacted context after Shell tool output without suppressing the original result.";
const TOKENJUICE_KIMI_DISABLED_ADVISORY = "Kimi support is beta; verify live PostToolUse behavior after install.";
function getKimiConfigDir(options = {}) {
...
function getLegacyJsonConfigPath(configPath) {
    return join(dirname(configPath), "config.json");
}
function tomlString(value) {
    return JSON.stringify(value);
}
function createKimiHookBlock(command) {
    return [
...
function appendKimiHookBlock(text, command) {
...
async function writeConfig(configPath, text) {
...
function getKimiHookBlock(text) {
...
function findTokenjuiceKimiHookCommand(text) {
...
export async function installKimiHook(configPath, options = {}) {
    ...
    if (!existing.exists) {
        ...
        if (legacyJsonConfig.exists) {
            throw new Error(`cannot install Kimi hook because ${legacyJsonConfigPath} exists and ${resolvedConfigPath} does not; start Kimi once to migrate config.json to config.toml, then rerun tokenjuice install kimi`);
        }
    }
    if (existing.exists && hasMalformedMarkerStructure(existing.text)) {
        throw new Error(`cannot safely repair malformed tokenjuice Kimi markers in ${resolvedConfigPath}; remove the dangling marker manually, then rerun tokenjuice install kimi`);
    }
...
export async function uninstallKimiHook(configPath = getDefaultConfigPath()) {
    const existing = await readConfig(configPath);
    if (!existing.exists) {
        return { configPath, removed: 0 };
    }
    if (hasMalformedMarkerStructure(existing.text)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice Kimi markers in ${configPath}; remove the dangling marker manually, then rerun tokenjuice uninstall kimi`);
    }
...
export async function doctorKimiHook(configPath, options = {}) {
    const resolvedConfigPath = configPath ?? getDefaultConfigPath(options);
...
function readKimiOutputText(output) {
...
export async function runKimiPostToolUseHook(rawText) {
    let payload;
    try {
