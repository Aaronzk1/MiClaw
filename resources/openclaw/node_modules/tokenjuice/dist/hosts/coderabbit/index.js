...
const TOKENJUICE_CODERABBIT_ADVISORY = "CodeRabbit support is beta and path-instruction based; CodeRabbit still owns pull-request review, tools, and comment delivery.";
function getExplicitProjectDir(options = {}) {
...
function isInsideOrEqual(parentDir, childPath) {
    const relativePath = relative(parentDir, childPath);
    return relativePath === "" || (!relativePath.startsWith("..") && !isAbsolute(relativePath));
}
function getExpectedConfigPath(projectDir) {
    return join(projectDir, ".coderabbit.yaml");
}
async function realpathExistingAncestor(path) {
    let current = path;
    while (true) {
        try {
...
async function rejectConfigSymlink(filePath) {
    try {
...
async function rejectInstallSidecarSymlinks(filePath) {
    await rejectConfigSymlink(`${filePath}.bak`);
    await rejectConfigSymlink(`${filePath}.tmp`);
}
async function rejectSymlinkPathComponents(filePath, projectDir) {
...
async function resolveSafeProjectConfigPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use CodeRabbit config ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
...
async function getDefaultConfigPath(options = {}) {
...
async function getDefaultAliasPath(options = {}) {
    return getExpectedConfigPath(await resolveProjectDir(options));
}
async function pathExistsWithoutReading(filePath) {
    try {
...
async function resolveConfigPath(configPath, options = {}) {
    if (configPath) {
        const projectDir = await resolveProjectDir(options);
        const realProjectDir = await realpath(projectDir).catch(() => projectDir);
...
function getYamlLineIndent(line) {
    return line.match(/^\s*/u)?.[0].length ?? 0;
}
function getYamlStructureLines(lines) {
...
function getStructuralMarkerLines(text, marker) {
...
function findTokenjuiceBlockLineRange(text) {
...
function hasMalformedMarkerStructure(text) {
    const begin = getStructuralMarkerLines(text, TOKENJUICE_CODERABBIT_BEGIN);
    const end = getStructuralMarkerLines(text, TOKENJUICE_CODERABBIT_END);
...
function hasStructuralTokenjuiceMarker(text) {
...
function hasTokenjuiceBlock(text) {
    return Boolean(findTokenjuiceBlockLineRange(text));
}
function getTokenjuiceBlockText(text) {
...
function removeTokenjuiceBlock(text) {
    const range = findTokenjuiceBlockLineRange(text);
    if (!range) {
...
function countTopLevelKey(lines, key) {
...
function isYamlBlockMappingSuffix(value) {
...
function findTopLevelMappingRange(lines, key) {
    const structureLines = getYamlStructureLines(lines);
...
function findNestedMappingRange(lines, parentRange, key, indent) {
    const structureLines = getYamlStructureLines(lines);
...
function findNestedKeyOccurrences(lines, parentRange, key) {
    const structureLines = getYamlStructureLines(lines);
...
function assertSupportedYamlShape(text, configPath) {
    const lines = getYamlStructureLines(text.split("\n"));
    const unsupportedReviewsKey = lines.some((line) => /^(?:"reviews"|'reviews'|reviews)\s*:/u.test(line) && !/^reviews:\s*/u.test(line));
    if (unsupportedReviewsKey) {
        throw new Error(`cannot install CodeRabbit guidance because ${configPath} uses an unsupported root reviews key shape; convert it to a plain reviews mapping, then rerun tokenjuice install coderabbit`);
    }
    if (countTopLevelKey(lines, "reviews") > 1) {
        throw new Error(`cannot install CodeRabbit guidance because ${configPath} has multiple top-level reviews keys; merge them manually, then rerun tokenjuice install coderabbit`);
    }
    ...
    if (inlineReviews) {
        throw new Error(`cannot install CodeRabbit guidance because ${configPath} defines reviews as an inline YAML value; convert it to a reviews mapping, then rerun tokenjuice install coderabbit`);
    }
    ...
    if (firstReviewsContent && !/^ {2}\S/u.test(firstReviewsContent)) {
        throw new Error(`cannot install CodeRabbit guidance because ${configPath} uses unsupported indentation inside reviews; convert reviews to two-space child indentation, then rerun tokenjuice install coderabbit`);
    }
    const unsupportedPathInstructionsKey = lines
        .slice(reviewsRange.start + 1, reviewsRange.end)
        .some((line) => /^ {2}(?:"path_instructions"|'path_instructions'|path_instructions)\s*:/u.test(line) &&
        !/^  path_instructions:\s*/u.test(line));
    if (unsupportedPathInstructionsKey) {
        throw new Error(`cannot install CodeRabbit guidance because ${configPath} uses an unsupported reviews.path_instructions key shape; convert it to a two-space reviews.path_instructions block, then rerun tokenjuice install coderabbit`);
    }
    ...
    if (duplicatePathInstructions) {
        throw new Error(`cannot install CodeRabbit guidance because ${configPath} has multiple reviews.path_instructions keys; merge them manually, then rerun tokenjuice install coderabbit`);
    }
    ...
    if (inlinePathInstructions) {
        throw new Error(`cannot install CodeRabbit guidance because ${configPath} defines reviews.path_instructions as an inline YAML value; convert it to a block list, then rerun tokenjuice install coderabbit`);
    }
    ...
    if (pathInstructionsRange) {
        ...
        if (firstPathInstructionsContent && !/^ {4}-\s/u.test(firstPathInstructionsContent)) {
            throw new Error(`cannot install CodeRabbit guidance because ${configPath} defines reviews.path_instructions as a non-list YAML block; convert it to a block list, then rerun tokenjuice install coderabbit`);
        }
    }
}
function collectSupportedYamlShapeIssues(text, configPath) {
    try {
...
function collectTokenjuiceBlockPlacementIssues(text) {
    const lines = text.split("\n");
    ...
    const isInsidePathInstructions = beginLine > pathInstructionsRange.start && endLine > beginLine && endLine < pathInstructionsRange.end;
...
const TOKENJUICE_CODERABBIT_GUIDANCE = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When CodeRabbit review, finishing-touch, chat, or tool workflows suggest terminal commands likely to produce long output, prefer `tokenjuice wrap -- <command>`.",
    }),
];
function createPathInstructionBlock(indent) {
    const prefix = " ".repeat(indent);
    return [
...
function createPathInstructionsBlock(indent) {
    const prefix = " ".repeat(indent);
    return [`${prefix}path_instructions:`, ...createPathInstructionBlock(indent + 2)];
}
function installTokenjuiceBlock(text, configPath) {
...
function removeEmptyCodeRabbitContainers(text) {
    if (!text.trim()) {
        return "";
    }
...
export async function installCodeRabbitConfig(configPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text)) {
        throw new Error(`cannot safely repair malformed tokenjuice CodeRabbit markers in ${resolvedConfigPath}; remove the dangling marker manually, then rerun tokenjuice install coderabbit`);
    }
...
export async function uninstallCodeRabbitConfig(configPath, options = {}) {
    const resolvedConfigPath = await resolveConfigPath(configPath, options);
    const existing = await readInstructionFile(resolvedConfigPath);
    if (!existing.exists) {
    ...
    if (hasMalformedMarkerStructure(existing.text)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice CodeRabbit markers in ${resolvedConfigPath}; remove the dangling marker manually, then rerun tokenjuice uninstall coderabbit`);
    }
...
export async function doctorCodeRabbitConfig(configPath, options = {}) {
    ...
    try {
        ...
        return {
            ...
            ...buildInstructionDoctorReportFields({
                ...
                advisory: TOKENJUICE_CODERABBIT_ADVISORY,
                fixCommand: error.message.includes("outside") || error.message.includes("only installs")
