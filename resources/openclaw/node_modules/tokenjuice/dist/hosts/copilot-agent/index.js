...
import { buildTokenjuiceHookCommand, isExecutableFile, pathExists, } from "../shared/host-command.js";
import { ensureHooksDir, isRecord, loadCopilotHooksConfigWithBackup, readCopilotHooksConfig, writeCopilotHooksConfigAtomic, } from "../shared/hooks-json-file.js";
...
const TOKENJUICE_COPILOT_AGENT_ADVISORY = "Copilot coding agent support is beta; cloud agent jobs must have tokenjuice available in PATH before the repo hook runs.";
const TOKENJUICE_COPILOT_AGENT_DISABLED_ADVISORY = "Copilot coding agent support is beta; install writes a repo-level .github/hooks hook for bash PostToolUse events.";
const GENERIC_FALLBACK_MIN_SAVED_CHARS = 120;
const GENERIC_FALLBACK_MAX_RATIO = 0.75;
async function buildCopilotAgentHookCommand(options = {}) {
    if (!options.local) {
...
async function resolveProjectDir(options = {}, requireRepositoryRoot = false) {
...
function isTokenjuiceCopilotAgentHook(entry) {
    return readHookCommand(entry) !== undefined;
}
function getPostToolUseHooks(config) {
    const postToolUse = config.hooks.postToolUse;
    return Array.isArray(postToolUse) ? postToolUse : [];
}
function removeTokenjuiceCopilotAgentHooks(config) {
    let removed = 0;
...
function isEffectivelyEmptyHooksConfig(config) {
    return Object.keys(config.hooks).length === 0
        && Object.keys(config).filter((key) => key !== "hooks" && key !== "version").length === 0;
}
async function writeOrDeleteHooksConfig(hooksPath, config) {
...
function findTokenjuiceCopilotAgentHook(config) {
...
function findTokenjuiceCopilotAgentHookCommand(config) {
    return readHookCommand(findTokenjuiceCopilotAgentHook(config));
}
function findAnyTokenjuiceCopilotAgentHookCommand(config) {
...
function findMisplacedTokenjuiceCopilotAgentHookEvents(config) {
    const events = [];
    for (const [eventName, entries] of Object.entries(config.hooks)) {
...
function createTokenjuiceCopilotAgentHook(command) {
    return {
        type: "command",
        bash: command,
        command,
...
async function findStrayCopilotAgentHooks(hooksDir, canonicalPath) {
    let entries;
    try {
...
async function uninstallStrayCopilotAgentHookFiles(hooksDir, canonicalPath) {
    let entries;
    try {
        entries = await readdir(hooksDir);
    }
...
export async function installCopilotAgentHook(hooksPath, options = {}) {
...
export async function uninstallCopilotAgentHook(hooksPath, options = {}) {
    const resolvedHooksPath = hooksPath ?? await getDefaultHooksPath(options, true);
...
export async function doctorCopilotAgentHook(hooksPath, options = {}) {
    ...
    const misplacedIssues = misplacedEvents.map((eventName) => `tokenjuice copilot-agent hook is configured outside postToolUse in ${eventName}; run ${fixCommand} to repair`);
    if (!detectedCommand) {
...
export async function runCopilotAgentPostToolUseHook(rawText) {
    let payload;
    try {
