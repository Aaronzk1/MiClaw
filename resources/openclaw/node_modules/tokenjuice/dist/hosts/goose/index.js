...
import { TOKENJUICE_FULL_COMMAND, TOKENJUICE_RAW_COMMAND, TOKENJUICE_WRAP_COMMAND, } from "../shared/instruction-guidance.js";
...
function getHintsPathForDir(dir) {
    return join(resolve(dir), GOOSE_HINTS_FILENAME);
}
async function getTokenjuiceHintsPathInDir(dir) {
...
async function findParentTokenjuiceHintsPaths(startDir, boundaryDir) {
    let current = resolve(startDir);
...
async function findProjectTokenjuiceHintsPaths(projectDir) {
...
async function getDefaultHintsPaths(options = {}, preferExistingTokenjuiceBlocks = false, includeRootWithExistingBlocks = false) {
    const projectDir = await resolveProjectDir(options);
    const rootHintsPath = getHintsPathForDir(projectDir);
    const existingHintsPaths = [];
...
export async function installGooseHints(hintsPath, options = {}) {
    ...
    for (const resolvedHintsPath of resolvedHintsPaths) {
        ...
        if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
            throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedHintsPath}; remove the dangling marker manually, then rerun tokenjuice install goose`);
        }
    }
    const results = [];
    for (const resolvedHintsPath of resolvedHintsPaths) {
...
export async function uninstallGooseHints(hintsPath, options = {}) {
    const resolvedHintsPaths = hintsPath ? [hintsPath] : await getDefaultHintsPaths(options, true);
    for (const resolvedHintsPath of resolvedHintsPaths) {
        const existing = await readInstructionFile(resolvedHintsPath);
        const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_GOOSE_BLOCK_CONFIG);
        if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
            throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedHintsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall goose`);
        }
    }
    const results = [];
    for (const resolvedHintsPath of resolvedHintsPaths) {
...
async function inspectGooseHintsPath(resolvedHintsPath) {
    const existing = await readInstructionFile(resolvedHintsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_GOOSE_BLOCK_CONFIG);
...
export async function doctorGooseHints(hintsPath, options = {}) {
    const resolvedHintsPaths = hintsPath ? [hintsPath] : await getDefaultHintsPaths(options, true);
    const reports = [];
    for (const resolvedHintsPath of resolvedHintsPaths) {
