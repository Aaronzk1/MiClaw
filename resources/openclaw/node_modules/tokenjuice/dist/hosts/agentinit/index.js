...
const TOKENJUICE_AGENTINIT_ADVISORY = "AgentInit support is beta and source-instruction based; run `agentinit sync` after install so generated agent files receive the guidance.";
...
async function resolveSafeProjectInstructionsPath(filePath, projectDir) {
    const realParentDir = await realpathExistingAncestor(dirname(filePath));
    if (!isInsideOrEqual(projectDir, realParentDir)) {
        throw new Error(`cannot use AgentInit source ${filePath}; tokenjuice will not write through instruction directories outside ${projectDir}`);
    }
...
const TOKENJUICE_AGENTINIT_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When AgentInit syncs this AGENTS.md into AI coding tool instruction files, prefer `tokenjuice wrap -- <command>` for terminal commands likely to produce long output.",
    }),
...
export async function installAgentInitInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install agentinit`);
    }
...
export async function uninstallAgentInitInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = await resolveInstructionsPath(instructionsPath, options);
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_AGENTINIT_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall agentinit`);
    }
...
export async function doctorAgentInitInstructions(instructionsPath, options = {}) {
