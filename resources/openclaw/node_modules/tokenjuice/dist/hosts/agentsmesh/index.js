...
const TOKENJUICE_AGENTSMESH_ADVISORY = "AgentsMesh support is beta and rule-based; run `agentsmesh init` before install and `agentsmesh generate` after install so generated tool configs receive the rule.";
const TOKENJUICE_AGENTSMESH_UNINITIALIZED_ISSUE = "AgentsMesh project is not initialized; run `agentsmesh init` before installing tokenjuice rules";
const TOKENJUICE_AGENTSMESH_RULES_DISABLED_ISSUE = "AgentsMesh rules feature is disabled in agentsmesh.yaml; add `rules` to features before running agentsmesh generate";
const AGENTSMESH_REQUIRED_PROJECT_FILES = ["agentsmesh.yaml"];
...
async function findAgentsMeshProjectRoot(startDir) {
    let current = resolve(startDir);
    while (true) {
...
async function getDefaultRulePath(options = {}) {
    return join(await resolveProjectDir(options), ".agentsmesh", "rules", "tokenjuice.md");
}
async function getAgentsMeshProjectState(projectDir) {
...
function normalizeYamlListValue(value) {
...
function parseYamlList(lines, startIndex, parentIndent) {
    const items = [];
...
function parseYamlFeatureValue(lines, index, value, parentIndent) {
...
function parseAgentsMeshConfig(text) {
    const lines = text.split(/\r?\n/);
    let topLevelFeatures;
    const overrideTargetsWithoutRules = [];
...
async function getAgentsMeshConfigState(projectDir) {
    const configPath = join(projectDir, "agentsmesh.yaml");
    return parseAgentsMeshConfig(await readFile(configPath, "utf8"));
}
async function assertInitializedAgentsMeshProject(projectDir) {
    ...
    if (projectState.missingPaths.length === 0) {
        ...
        throw new Error(`cannot install AgentsMesh rule because ${join(projectDir, "agentsmesh.yaml")} disables the rules feature; add rules to features, then rerun tokenjuice install agentsmesh`);
    }
    throw new Error(`cannot install AgentsMesh rule because ${projectDir} is not initialized for AgentsMesh; run agentsmesh init first, then rerun tokenjuice install agentsmesh`);
}
function withProjectState(fields, projectState) {
    if (!projectState) {
        return fields;
    }
    return {
...
export async function installAgentsMeshRule(rulePath, options = {}) {
...
export async function uninstallAgentsMeshRule(rulePath, options = {}) {
    ...
    if (existing.exists
        ...
        throw new Error(`refusing to remove ${resolvedRulePath}; it does not look like the tokenjuice AgentsMesh rule. Review and remove it manually, or reinstall tokenjuice agentsmesh first.`);
    }
...
export async function doctorAgentsMeshRule(rulePath, options = {}) {
    ...
    const resolvedRulePath = rulePath ?? join(projectDir ?? (await resolveProjectDir(options)), ".agentsmesh", "rules", "tokenjuice.md");
    ...
    const configIssues = [
        ...
        ...(configState?.overrideTargetsWithoutRules.map((target) => `AgentsMesh target override disables rules for ${target}; generated config for that target will not receive tokenjuice guidance`) ?? []),
    ];
    const existing = await readInstructionFile(resolvedRulePath);
    if (!existing.exists) {
        ...
        return {
            ...
            ...withProjectState(buildInstructionDoctorReportFields({
                status,
                issues: [
    ...
    const status = issues.length > 0 || !isProjectInitialized || configState?.rulesEnabled === false
        ? "broken"
