...
const TOKENJUICE_SWE_AGENT_ADVISORY = "SWE-agent support is beta and config-fragment based; load it with sweagent run --config config/default.yaml --config .swe-agent/tokenjuice.yaml.";
function isTokenjuiceSweAgentConfigText(text) {
...
async function chooseSweAgentBackupPath(configPath) {
...
async function writeSweAgentConfigWithoutBackup(configPath, text) {
...
function buildSweAgentConfig({ restoreBackupSuffix } = {}) {
    return [
        ...
        ...buildTokenjuiceGuidanceBullets({
            wrapBullet: `- For SWE-agent bash commands likely to produce long output, run them through \`${TOKENJUICE_WRAP_COMMAND}\`.`,
        }).map((bullet) => `# ${bullet}`),
        "",
    ].join("\n");
}
export async function installSweAgentConfig(configPath, options = {}) {
    ...
    if (existing.exists) {
        ...
        await writeSweAgentConfigWithoutBackup(resolvedConfigPath, buildSweAgentConfig({ restoreBackupSuffix: backupPath.slice(resolvedConfigPath.length) }));
...
export async function uninstallSweAgentConfig(configPath, options = {}) {
    const resolvedConfigPath = configPath ?? (await getDefaultConfigPath(options));
    const existing = await readInstructionFile(resolvedConfigPath);
    if (!existing.exists || !isTokenjuiceSweAgentConfigText(existing.text)) {
...
export async function doctorSweAgentConfig(configPath, options = {}) {
    const resolvedConfigPath = configPath ?? (await getDefaultConfigPath(options));
    const existing = await readInstructionFile(resolvedConfigPath);
    if (!existing.exists) {
        return {
