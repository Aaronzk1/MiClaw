...
const TOKENJUICE_AGENTS_CLI_ADVISORY = "agents-cli support is beta and memory-based; agents-cli syncs memory into downstream harness configs, but each harness still owns command execution.";
function getAgentsCliConfigDir(options = {}) {
    return resolve(options.configDir || process.env.AGENTS_CLI_HOME || join(homedir(), ".agents"));
}
function getDefaultInstructionsPath(options = {}) {
...
const TOKENJUICE_AGENTS_CLI_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When agents-cli syncs this memory into coding-agent harnesses, prefer `tokenjuice wrap -- <command>` for terminal commands likely to produce long output.",
    }),
...
export async function installAgentsCliMemory(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install agents-cli`);
    }
...
export async function uninstallAgentsCliMemory(instructionsPath, options = {}) {
    const resolvedInstructionsPath = instructionsPath ?? getDefaultInstructionsPath(options);
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_AGENTS_CLI_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall agents-cli`);
    }
...
export async function doctorAgentsCliMemory(instructionsPath, options = {}) {
    const resolvedInstructionsPath = instructionsPath ?? getDefaultInstructionsPath(options);
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_AGENTS_CLI_BLOCK_CONFIG);
