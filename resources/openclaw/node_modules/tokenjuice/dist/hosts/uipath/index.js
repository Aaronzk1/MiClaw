...
const TOKENJUICE_UIPATH_ADVISORY = "UiPath support is beta and instruction-file based; the active coding agent still owns command execution.";
...
const TOKENJUICE_UIPATH_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When running terminal commands through UiPath for Coding Agents, prefer `tokenjuice wrap -- <command>` for commands likely to produce long output.",
    }),
...
export async function installUiPathInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install uipath`);
    }
...
export async function uninstallUiPathInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = instructionsPath ?? (await getDefaultInstructionsPath(options));
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_UIPATH_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall uipath`);
    }
...
export async function doctorUiPathInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = instructionsPath ?? (await getDefaultInstructionsPath(options));
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_UIPATH_BLOCK_CONFIG);
