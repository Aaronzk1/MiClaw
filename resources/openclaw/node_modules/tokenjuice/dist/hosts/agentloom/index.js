...
const TOKENJUICE_AGENTLOOM_ADVISORY = "Agentloom support is beta and rule-based; run `agentloom sync` after install so provider-native configs receive the rule.";
...
export async function installAgentloomRule(rulePath, options = {}) {
...
export async function uninstallAgentloomRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
...
export async function doctorAgentloomRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
    const existing = await readInstructionFile(resolvedRulePath);
    if (!existing.exists) {
        return {
