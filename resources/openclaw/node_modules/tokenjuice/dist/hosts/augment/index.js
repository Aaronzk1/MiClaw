...
export async function uninstallAugmentRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
    const existing = await readInstructionFile(resolvedRulePath);
    if (existing.exists && !isTokenjuiceAugmentRuleText(existing.text)) {
        throw new Error(`refusing to remove ${resolvedRulePath}; it does not look like the tokenjuice Augment rule. Review and remove it manually, or reinstall tokenjuice augment first.`);
    }
...
export async function doctorAugmentRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
    const existing = await readInstructionFile(resolvedRulePath);
    if (!existing.exists) {
        return {
