...
import { collectMarkerDelimitedBlockIssues, inspectMarkerDelimitedBlock, installMarkerDelimitedBlock, removeMarkerDelimitedBlock, uninstallMarkerDelimitedBlock, } from "../shared/marker-instructions.js";
...
const TOKENJUICE_ANYWHERE_AGENTS_ADVISORY = "anywhere-agents support is beta and local-override based; anywhere-agents composes AGENTS.md plus AGENTS.local.md into downstream agent files, but each agent still owns command execution. Run `anywhere-agents` after install or uninstall.";
...
async function resolveSafeProjectWritePath(filePath, projectDir) {
    const realParentDir = await realpathExistingAncestor(dirname(filePath));
    if (!isInsideOrEqual(projectDir, realParentDir)) {
        throw new Error(`cannot use anywhere-agents source ${filePath}; tokenjuice will not write through instruction directories outside ${projectDir}`);
    }
    let stats;
    try {
    ...
    if (!isInsideOrEqual(projectDir, targetPath)) {
        throw new Error(`cannot use anywhere-agents source ${filePath}; tokenjuice will not follow instruction symlinks outside ${projectDir}`);
    }
    return { filePath: targetPath, followedSymlink: true };
}
async function getDefaultInstructionsTarget(options = {}) {
...
const TOKENJUICE_ANYWHERE_AGENTS_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When anywhere-agents layers this AGENTS.local.md after the generated AGENTS.md and deploys downstream agent files, prefer `tokenjuice wrap -- <command>` for terminal commands likely to produce long output.",
    }),
...
export async function installAnywhereAgentsInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install anywhere-agents`);
    }
...
export async function uninstallAnywhereAgentsInstructions(instructionsPath, options = {}) {
    ...
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_ANYWHERE_AGENTS_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall anywhere-agents`);
    }
...
export async function doctorAnywhereAgentsInstructions(instructionsPath, options = {}) {
