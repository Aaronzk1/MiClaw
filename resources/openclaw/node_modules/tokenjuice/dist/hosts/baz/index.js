...
const TOKENJUICE_BAZ_ADVISORY = "Baz support is beta and skill-based; Baz discovers Skill folders containing SKILL.md and converts them into AI Coding Guidelines, but tokenjuice does not intercept Baz review output.";
function isTokenjuiceBazSkillText(text) {
...
async function resolveSafeProjectSkillPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use Baz skill ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
...
const TOKENJUICE_BAZ_SKILL = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When Baz agents run terminal commands likely to produce long output during review, fixer, or runtime inspection workflows, prefer `tokenjuice wrap -- <command>`.",
    }),
...
export async function installBazSkill(skillPath, options = {}) {
...
export async function uninstallBazSkill(skillPath, options = {}) {
    const resolvedSkillPath = await resolveSkillPath(skillPath, options);
...
export async function doctorBazSkill(skillPath, options = {}) {
