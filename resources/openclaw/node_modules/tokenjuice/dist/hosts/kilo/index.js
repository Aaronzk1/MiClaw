...
function getRootConfigPath(options = {}) {
    return join(getProjectDir(options), "kilo.jsonc");
}
function getDotConfigPath(options = {}) {
...
function getProjectConfigPaths(options = {}) {
    return [getDotConfigPath(options), getRootConfigPath(options)];
}
function getInstructionRefForRule(rulePath, options = {}) {
...
function readJsonString(text, start) {
    ...
    while (cursor < text.length) {
        ...
        if (char === "\"") {
            ...
            try {
                return { end: cursor, value: JSON.parse(raw) };
            }
            catch {
                return null;
            }
...
function skipJsonValue(text, index, limit) {
...
function findLastSignificantChar(text, start, end) {
    let cursor = start;
    let last;
    while (cursor < end) {
        cursor = skipWhitespaceAndComments(text, cursor);
...
function findInstructionsArray(text) {
...
function hasKiloInstructionRef(text, location, instructionRef) {
...
function hasAnyInstructionsValue(text, location) {
    if (location.kind !== "array") {
        return false;
    }
    return findLastSignificantChar(text, location.arrayStart + 1, location.arrayEnd) !== undefined;
}
function ensureKiloConfigInstruction(text, instructionRef) {
...
function removeKiloConfigInstruction(text, instructionRef) {
...
async function writeTextFileWithBackup(filePath, text) {
...
async function ensureKiloConfig(configPath, instructionRef) {
...
async function removeKiloConfigRef(configPath, instructionRef) {
    const existing = await readInstructionFile(configPath);
    if (!existing.exists) {
        return { configPath, updated: false };
    }
...
export async function installKiloRule(rulePath, options = {}) {
...
export async function uninstallKiloRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? getDefaultRulePath(options);
    const resolvedConfigPath = await getDefaultConfigPath(options);
    const instructionRef = getInstructionRefForRule(resolvedRulePath, options);
...
export async function doctorKiloRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? getDefaultRulePath(options);
    const resolvedConfigPath = await getDefaultConfigPath(options);
    const instructionRef = getInstructionRefForRule(resolvedRulePath, options);
    const existing = await readInstructionFile(resolvedRulePath);
    const config = await readInstructionFile(resolvedConfigPath);
    const configLocation = config.exists ? findInstructionsArray(config.text) : { kind: "missing" };
