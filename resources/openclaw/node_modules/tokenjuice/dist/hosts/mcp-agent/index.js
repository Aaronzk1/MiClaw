...
const TOKENJUICE_MCP_AGENT_ADVISORY = "mcp-agent support is beta and agent-file based; enable `.mcp-agent/agents` in `agents.search_paths` so mcp-agent can load it.";
function isTokenjuiceMcpAgentDefinitionText(text) {
...
async function chooseMcpAgentBackupPath(agentPath) {
...
async function writeMcpAgentDefinitionWithoutBackup(agentPath, text) {
...
async function rejectDefinitionSymlink(filePath) {
    try {
...
async function resolveSafeProjectAgentPath(filePath, projectDir) {
    const realParentDir = await realpathExistingAncestor(dirname(filePath));
    if (!isInsideOrEqual(projectDir, realParentDir)) {
        throw new Error(`cannot use mcp-agent definition ${filePath}; tokenjuice will not write through instruction directories outside ${projectDir}`);
    }
    await rejectDefinitionSymlink(filePath);
    return filePath;
}
async function getDefaultAgentPath(options = {}) {
...
async function resolveAgentPath(agentPath, options = {}) {
    if (agentPath) {
        const projectDir = await resolveProjectDir(options);
        const realProjectDir = await realpath(projectDir).catch(() => projectDir);
...
function buildMcpAgentDefinition({ restoreBackupSuffix } = {}) {
    return [
        "---",
        "name: tokenjuice",
        ...
        ...buildTokenjuiceGuidanceBullets({
            wrapBullet: `- When an mcp-agent workflow or subagent runs terminal commands likely to produce long output, prefer \`${TOKENJUICE_WRAP_COMMAND}\`.`,
        }),
        "",
...
export async function installMcpAgentDefinition(agentPath, options = {}) {
    ...
    if (existing.exists) {
        ...
        await writeMcpAgentDefinitionWithoutBackup(resolvedAgentPath, buildMcpAgentDefinition({ restoreBackupSuffix: backupPath.slice(resolvedAgentPath.length) }));
...
export async function uninstallMcpAgentDefinition(agentPath, options = {}) {
    const resolvedAgentPath = await resolveAgentPath(agentPath, options);
    const existing = await readInstructionFile(resolvedAgentPath);
    if (!existing.exists || !isTokenjuiceMcpAgentDefinitionText(existing.text)) {
...
export async function doctorMcpAgentDefinition(agentPath, options = {}) {
