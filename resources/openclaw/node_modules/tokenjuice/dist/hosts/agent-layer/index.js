...
const TOKENJUICE_AGENT_LAYER_ADVISORY = "Agent Layer support is beta and instruction-based; run `al init` before install and `al sync` after install so generated client files receive the guidance.";
const TOKENJUICE_AGENT_LAYER_UNINITIALIZED_ISSUE = "Agent Layer project is not initialized; run `al init` before installing tokenjuice instructions";
const AGENT_LAYER_REQUIRED_PROJECT_FILES = [".agent-layer/config.toml", ".agent-layer/al.version"];
...
async function getAgentLayerProjectState(projectDir) {
...
async function assertInitializedAgentLayerProject(projectDir) {
    const projectState = await getAgentLayerProjectState(projectDir);
    if (projectState.missingPaths.length === 0) {
        return;
    }
    throw new Error(`cannot install Agent Layer instructions because ${projectDir} is not initialized for Agent Layer; run al init first, then rerun tokenjuice install agent-layer`);
}
...
export async function installAgentLayerInstructions(instructionsPath, options = {}) {
...
export async function uninstallAgentLayerInstructions(instructionsPath, options = {}) {
...
export async function doctorAgentLayerInstructions(instructionsPath, options = {}) {
    ...
    const resolvedInstructionsPath = instructionsPath ?? join(projectDir ?? (await resolveProjectDir(options)), ".agent-layer", "instructions", "tokenjuice.md");
