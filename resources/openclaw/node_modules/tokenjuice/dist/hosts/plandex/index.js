...
async function getDefaultConventionPath(options = {}) {
...
function isTokenjuicePlandexConventionText(text) {
    return text.includes(TOKENJUICE_PLANDEX_MARKER);
}
async function writePlandexConventionWithoutBackup(conventionPath) {
...
export async function installPlandexConvention(conventionPath, options = {}) {
...
export async function uninstallPlandexConvention(conventionPath, options = {}) {
    const resolvedConventionPath = conventionPath ?? await getDefaultConventionPath(options);
    const existing = await readInstructionFile(resolvedConventionPath);
    if (existing.exists && !isTokenjuicePlandexConventionText(existing.text)) {
        throw new Error(`refusing to remove ${resolvedConventionPath}; it does not look like the tokenjuice Plandex convention. Review and remove it manually, or reinstall tokenjuice plandex first.`);
    }
...
export async function doctorPlandexConvention(conventionPath, options = {}) {
    const resolvedConventionPath = conventionPath ?? await getDefaultConventionPath(options);
    const existing = await readInstructionFile(resolvedConventionPath);
    if (!existing.exists) {
        return {
