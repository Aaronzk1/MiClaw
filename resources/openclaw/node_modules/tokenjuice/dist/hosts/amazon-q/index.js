...
const TOKENJUICE_AMAZON_Q_ADVISORY = "Amazon Q support is beta and rule-based; Amazon Q CLI has been rebranded to Kiro, but Kiro can continue using Amazon Q rules. Include `file://.amazonq/rules/**/*.md` in the active agent resources so the rule is loaded.";
...
export async function uninstallAmazonQRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? await getDefaultRulePath(options);
    const existing = await readInstructionFile(resolvedRulePath);
    if (existing.exists && !isTokenjuiceAmazonQRuleText(existing.text)) {
        throw new Error(`refusing to remove ${resolvedRulePath}; it does not look like the tokenjuice Amazon Q rule. Review and remove it manually, or reinstall tokenjuice amazon-q first.`);
    }
...
export async function doctorAmazonQRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? await getDefaultRulePath(options);
    const existing = await readInstructionFile(resolvedRulePath);
    if (!existing.exists) {
        return {
