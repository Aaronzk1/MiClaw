...
const TOKENJUICE_DOCKER_AGENT_ADVISORY = "Docker Agent support is beta and prompt-file based; add `.docker-agent/tokenjuice.md` to the agent config's `add_prompt_files` list.";
function isTokenjuiceDockerAgentPromptText(text) {
...
async function chooseDockerAgentBackupPath(promptPath) {
...
async function writeDockerAgentPromptWithoutBackup(promptPath, text) {
...
async function getDefaultPromptPath(options = {}) {
    return join(await resolveProjectDir(options), ".docker-agent", "tokenjuice.md");
}
function buildDockerAgentPrompt({ restoreBackupSuffix } = {}) {
    return [
        ...
        ...buildTokenjuiceGuidanceBullets({
            wrapBullet: `- When Docker Agent runs shell commands likely to produce long output, prefer \`${TOKENJUICE_WRAP_COMMAND}\`.`,
        }),
        "",
...
export async function installDockerAgentPrompt(promptPath, options = {}) {
    ...
    if (existing.exists && isTokenjuiceDockerAgentPromptText(existing.text)) {
        ...
        const result = await writeInstructionFile(resolvedPromptPath, buildDockerAgentPrompt({ restoreBackupSuffix }));
        return {
    ...
    if (existing.exists) {
        ...
        await writeDockerAgentPromptWithoutBackup(resolvedPromptPath, buildDockerAgentPrompt({ restoreBackupSuffix: backupPath.slice(resolvedPromptPath.length) }));
...
export async function uninstallDockerAgentPrompt(promptPath, options = {}) {
    const resolvedPromptPath = promptPath ?? (await getDefaultPromptPath(options));
    const existing = await readInstructionFile(resolvedPromptPath);
    if (!existing.exists || !isTokenjuiceDockerAgentPromptText(existing.text)) {
...
export async function doctorDockerAgentPrompt(promptPath, options = {}) {
    const resolvedPromptPath = promptPath ?? (await getDefaultPromptPath(options));
    const existing = await readInstructionFile(resolvedPromptPath);
    if (!existing.exists) {
        return {
