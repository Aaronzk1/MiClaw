...
function rewriteSearchLines(lines, noOmit = false) {
  ...
  const rewritten = lines.map((line) => {
    ...
    if (!match) {
      const clipped2 = clipMiddleWithHash(line, LONG_SEARCH_LINE_MAX_CHARS, noOmit);
...
function rewriteGhLines(lines, input, noOmit = false) {
  ...
  if (parsedJsonLines.every((entry) => entry !== null)) {
    const compactions = [];
    const rewritten = parsedJsonLines.map((entry) => formatGhJsonRecord(entry, noOmit)).flatMap((line) => {
