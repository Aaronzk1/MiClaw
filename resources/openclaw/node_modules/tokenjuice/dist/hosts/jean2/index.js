...
const TOKENJUICE_JEAN2_ADVISORY = "Jean2 support is beta and instruction-file based; Jean2 still owns command execution.";
...
const TOKENJUICE_JEAN2_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When running terminal commands through Jean2, prefer `tokenjuice wrap -- <command>` for commands likely to produce long output.",
    }),
...
export async function installJean2Instructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install jean2`);
    }
...
export async function uninstallJean2Instructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = instructionsPath ?? (await getDefaultInstructionsPath(options));
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_JEAN2_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall jean2`);
    }
...
export async function doctorJean2Instructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = instructionsPath ?? (await getDefaultInstructionsPath(options));
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_JEAN2_BLOCK_CONFIG);
