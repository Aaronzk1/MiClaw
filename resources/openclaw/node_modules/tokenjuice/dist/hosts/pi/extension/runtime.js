// src/hosts/pi/extension/runtime.ts
...
var SETUP_WRAPPER_COMMANDS = /* @__PURE__ */ new Set(["cd", "pwd", "set", "source", ".", "export", "unset", "trap", "true"]);
...
function splitTopLevelOrChain(command) {
  const trimmed = command.trim();
  if (!trimmed) {
    return [];
  }
...
function isFullyParenthesized(command) {
...
function stripSetupShellDecorators(command) {
...
function isSimpleSetupWrapperSegment(argv) {
  if (argv.length === 0) {
    return true;
  }
  const argv0 = getArgv0Name(argv);
...
function isQuietCommandProbe(argv) {
  const argv0 = getArgv0Name(argv);
  return argv0 === "command" && (argv[1] === "-v" || argv[1] === "-V") && argv.slice(2).some(redirectsStdout);
}
function redirectsStdout(arg) {
  return arg === ">" || arg === ">>" || arg === "1>" || arg === "1>>" || arg === "&>" || arg === "&>>" || /^(?:>|>>|1>|1>>|&>|&>>)\S/u.test(arg);
}
function isFailFastSetupGuard(argv) {
  const argv0 = getArgv0Name(argv);
  return argv0 === "exit" || argv0 === "return";
}
function isSetupWrapperSegment(argv, command) {
...
function isSetupWrapperCommand(command) {
  const segments = splitTopLevelCommandChain(command);
  return segments.length > 0 && segments.every((segment) => isSetupWrapperSegment(tokenizeCommand(segment), segment));
}
function isSingleSetupConditionSegment(argv, command) {
  const argv0 = getArgv0Name(argv);
  return argv0 === "[" || argv0 === "[[" || argv0 === "test" || isSetupWrapperSegment(argv, command);
}
function isSetupConditionSegment(argv, command) {
  const normalizedCommand = command ? stripSetupShellDecorators(command) : "";
  const orSegments = normalizedCommand ? splitTopLevelOrChain(normalizedCommand) : [];
  if (orSegments.length > 1) {
    return orSegments.every((segment) => {
...
function isSetupConditionCommand(command) {
  const segments = splitTopLevelCommandChain(command);
  return segments.length > 0 && segments.every((segment) => isSetupConditionSegment(tokenizeCommand(segment), segment));
}
function stripLeadingSetupIfBlock(command) {
...
function stripLeadingSetupSegment(command) {
  const trimmed = command.trim();
...
function resolveEffectiveCommand(input) {
  ...
  if (effectiveCommand !== command) {
    return resolveEffectiveCommand({ command: effectiveCommand }) ?? buildEffectiveCandidate(tokenizeCommand(effectiveCommand), true, effectiveCommand);
  }
  ...
  for (const segment of segments) {
    ...
    const effectiveCommand2 = stripLeadingEnvAssignmentsFromCommand(trimmedSegment) ?? void 0;
    const candidate = buildEffectiveCandidate(segmentArgv, transformedByChain, effectiveCommand2);
    if (candidate) {
      return candidate;
    }
  }
...
// src/core/env.ts
var NO_OMISSION_ENV = "TOKENJUICE_NO_OMISSION";
var NO_OMISSION_TRUTHY_VALUES = /* @__PURE__ */ new Set(["1", "true", "TRUE", "yes", "YES"]);
...
function createPassthroughCompactionMetadata(...kinds) {
...
function headTail(lines, head, tail, noOmit = false) {
...
function clampTextWithMetadata(text, maxChars, noOmit = false) {
  if (countTextChars(text) <= maxChars) {
    return { text };
  }
  if (noOmit) {
...
function clampTextMiddleWithMetadata(text, maxChars, noOmit = false) {
  if (countTextChars(text) <= maxChars) {
    return { text };
  }
  if (noOmit) {
...
function clipMiddleWithHash(text, maxChars, noOmit = false) {
  if (countTextChars(text) <= maxChars) {
    return { text };
  }
  if (noOmit) {
...
function compactWholeJsonText(rawText, maxChars, noOmit = false) {
  return parseJsonValue(rawText) === null ? null : clipMiddleWithHashStrict(minifyJsonLexically(rawText), maxChars, noOmit);
}
...
function buildPackageLockSummary(value, noOmit = false) {
...
function buildLargeDocumentSummary(lines, rawText, noOmit = false) {
  const headings = lines.map((line2) => line2.trim()).filter((line2) => /^#{1,6}\s+\S/u.test(line2)).slice(0, 24);
  const excerpt = headTail(lines, 6, 6, noOmit);
  const clippedHeadingCompactions = [];
  const clippedHeadings = headings.map((line2) => {
    const clipped = clipMiddleWithHash(line2, 180, noOmit);
  ...
  const excerptLines = excerpt.lines.map((line2) => {
    const clipped = clipMiddleWithHash(line2, 180, noOmit);
    if (clipped.compaction) {
...
function isLikelyDocumentLine(line2) {
  const trimmed = line2.trim();
...
function isLikelyCodeLine(line2) {
  const trimmed = line2.trim();
  if (!trimmed) {
    return false;
  }
...
function isLargeDocumentOutput(lines, rawChars) {
  ...
  const nonEmptyLines = lines.filter((line2) => line2.trim() !== "");
  const headingCount = nonEmptyLines.filter((line2) => /^#{1,6}\s+\S/u.test(line2.trim())).length;
  const hasFrontmatter = nonEmptyLines[0]?.trim() === "---" && nonEmptyLines.slice(1, 20).some((line2) => line2.trim() === "---");
...
function buildInspectionSummary(input, rawText, noOmit = false) {
  ...
  if (PACKAGE_LOCK_RE.test(command)) {
    const lines2 = buildPackageLockSummary(parseJsonValue(rawText), noOmit);
...
function isGithubActionsMetadataLine(line2) {
  return line2 === "##[endgroup]" || line2.startsWith("shell:") || line2.startsWith("env:") || line2.startsWith("Error: Process completed with exit code ");
}
function isShellSetupLine(line2) {
  return line2 === "set -e" || line2 === "set -eo pipefail" || line2 === "set -euo pipefail" || /^set\s+-[A-Za-z]*e[A-Za-z]*$/u.test(line2);
}
function stripGithubActionsScriptPrefix(line2) {
  return line2.replace(/^\s{2}/u, "").replace(/^\t/u, "").trim();
}
...
function extractRunCommands(lines) {
  const runIndex = lines.findIndex((line2) => GITHUB_ACTIONS_RUN_PATTERN.test(stripGithubActionsScriptPrefix(line2)));
  if (runIndex === -1) {
    return [];
  }
  const commands = [];
  ...
  const hasIndentedScriptBody = bodyLines.some((line2) => /^(?:\s{2}|\t)\S/u.test(line2));
  for (const rawLine of bodyLines) {
    if (hasIndentedScriptBody && !/^(?:\s{2}|\t)/u.test(rawLine)) {
      break;
    }
    const line2 = stripGithubActionsScriptPrefix(rawLine);
    if (!line2 || isGithubActionsMetadataLine(line2)) {
      break;
    }
    commands.push(line2);
  }
  return dedupeAdjacent2(commands).filter((line2) => !isShellSetupLine(line2));
}
function formatCommandList(commands, noOmit = false) {
...
function buildGithubActionsFailureSummary(input, rawText, noOmit = false) {
  const lines = trimEmptyEdges(normalizeLines(stripAnsi(rawText))).map((line2) => line2.trimEnd());
  const exitLine = [...lines].reverse().find((line2) => GITHUB_ACTIONS_EXIT_PATTERN.test(stripGithubActionsScriptPrefix(line2)));
...
function formatGhJsonRecord(record, noOmit = false) {
  ...
  const status = getGhString(record, "state") ?? getGhString(record, "conclusion") ?? getGhString(record, "status");
...
function formatGhCommentJsonRecord(record, noOmit = false) {
  ...
  const line2 = typeof record.line === "number" ? record.line : typeof record.originalLine === "number" ? record.originalLine : typeof record.startLine === "number" ? record.startLine : null;
  ...
  const location = path ? `${path}${line2 !== null ? `:${line2}` : ""}` : null;
  const parts = [
    "comment",
    id ? `#${id}` : null,
    author ? `@${author}` : null,
...
function formatGhStatusCheckRollup(value, noOmit = false) {
  ...
  for (const record of listed) {
    const line2 = formatGhStatusCheckLine(record);
    if (line2) {
      lines.push(line2);
    }
  }
  ...
  const compaction = noOmit ? okCount > 0 || omittedAttention > 0 ? createPassthroughCompactionMetadata("no-omit-domain-passthrough") : void 0 : okCount > 0 || omittedAttention > 0 ? createCompactionMetadata("github-status-check-rollup-omission") : void 0;
  return {
    lines,
    ...compaction && compaction.kinds.length > 0 ? { compaction } : {}
  };
}
function formatGhJsonValue(value, noOmit = false) {
  const compactions = [];
  if (Array.isArray(value)) {
    const lines2 = value.flatMap((entry) => {
      ...
      const formatted = formatGhJsonRecord(entry, noOmit);
      if (!formatted) {
        return [];
      }
...
function rewriteSearchLines(lines, noOmit = false) {
  const compactions = [];
  const rewritten = lines.map((line2) => {
    const match = /^(.+?:\d+(?::|-))(.*)$/u.exec(line2);
    if (!match) {
      const clipped2 = clipMiddleWithHash(line2, LONG_SEARCH_LINE_MAX_CHARS, noOmit);
...
function rewriteGitDiffLines(lines, noOmit = false) {
  const rewritten = [];
  const compactions = [];
  ...
  for (const line2 of lines) {
    ...
    const isChangedLine = line2.startsWith("+") || line2.startsWith("-");
    if (!isChangedLine || line2.startsWith("+++") || line2.startsWith("---")) {
      rewritten.push(line2);
      continue;
    }
    if (noOmit || emittedChangedLinesInHunk < GIT_DIFF_CHANGED_LINES_PER_HUNK) {
      ...
      const clipped = clipMiddleWithHash(line2, LONG_CHANGED_LINE_MAX_CHARS, noOmit);
      if (clipped.compaction) {
        compactions.push(clipped.compaction);
      }
...
function formatGhTableLine(line2) {
  ...
  const tabColumns = line2.split("	").map((part) => compactWhitespace(part)).filter(Boolean);
...
function isGhRunLogCommand(input) {
  const argv = input.argv ?? [];
  return isGithubCliCommand(argv[0]) && argv.includes("run") && argv.includes("view") && argv.some((arg) => arg === "--log" || arg === "--log-failed" || arg.startsWith("--log="));
}
function formatOmittedGhLogLines(count) {
  return `... ${count} non-signal log lines omitted ...`;
}
function filterGhRunLogSignalLines(lines, noOmit = false) {
  const signalIndexes = lines.map((line2, index) => GH_RUN_LOG_SIGNAL_PATTERN.test(line2) ? index : -1).filter((index) => index >= 0);
...
function rewriteGhLines(lines, input, noOmit = false) {
  ...
  if (parsedJsonLines.every((entry) => entry !== null)) {
    const compactions = [];
    const rewritten = parsedJsonLines.map((entry) => formatGhJsonRecord(entry, noOmit)).flatMap((line2) => {
      if (!line2 || line2.line.length === 0) {
        return [];
      }
      if (line2.compaction) {
        compactions.push(line2.compaction);
      }
      return [line2.line];
    });
...
function applyRule(compiledRule, input, rawText, noOmit = false) {
  ...
  for (const counter of compiledRule.compiled.counters) {
    ...
    if (rule.id === "git/diff" && (counter.name === "added line" || counter.name === "removed line")) {
      factLines = factLines.filter((line2) => !line2.startsWith("+++") && !line2.startsWith("---"));
    }
    facts[counter.name] = factLines.filter((line2) => pattern.test(line2)).length;
  }
...
function formatInline(classification, input, summary, facts, noOmit = false) {
  ...
  const shouldIncludeFacts = classification.family === "search" || classification.family !== "git-status" && classification.family !== "help" && (noOmit || summary.includes("omitted")) || classification.family === "test-results" && (input.exitCode ?? 0) !== 0;
...
function selectInlineText(classification, input, rawText, compactText, maxInlineChars, compactCompaction, noOmit = false) {
...
async function reduceExecutionWithRules(input, rules, opts = {}) {
  ...
  const buildStats = (reducedChars2) => ({
  ...
  if (inspectionSummary) {
    ...
    const selectedText2 = clampTextMiddleWithMetadata(summaryText, maxInlineChars, opts.noOmit);
  ...
  const githubActionsFailureSummary = classification.matchedReducer === "generic/fallback" ? buildGithubActionsFailureSummary(reducerInput, rawText, opts.noOmit) : null;
  if (githubActionsFailureSummary) {
    const inlineText2 = clampTextMiddleWithMetadata(githubActionsFailureSummary.text, maxInlineChars, opts.noOmit);
...
` : "";
    ...
    if (jsonOutput) {
      const inlineText2 = `${exitPrefix}${jsonOutput.text}`;
      const reducedChars2 = countTextChars(inlineText2);
      ...
      const stats2 = buildStats(reducedChars2);
      const rawRef2 = opts.store ? await storeArtifact(
        {
          input: normalizedInput,
          rawText,
...
// src/hosts/pi/extension/pi-types.ts
function isRecord2(value) {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

// src/hosts/pi/extension/settings.ts
...
// src/hosts/pi/extension/status.ts
...
// src/hosts/pi/extension/status-panel.ts
var ANSI_SGR_PATTERN = new RegExp("\\u001B\\[[0-9;]*m", "g");
...
function extractTextContent(content) {
  ...
  return content.filter((part) => isRecord3(part) && part.type === "text" && typeof part.text === "string").map((part) => String(part.text)).join("\n");
}
...
// src/hosts/pi/extension/tool-result.ts
