...
function isTokenjuiceAntigravityRuleText(text) {
    return text.includes(TOKENJUICE_ANTIGRAVITY_OWNERSHIP_MARKER);
}
async function writeAntigravityRuleWithoutBackup(rulePath) {
...
export async function installAntigravityRule(rulePath, options = {}) {
...
export async function uninstallAntigravityRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
    const existing = await readInstructionFile(resolvedRulePath);
    if (existing.exists && !isTokenjuiceAntigravityRuleText(existing.text)) {
        throw new Error(`refusing to remove ${resolvedRulePath}; it does not look like the tokenjuice Antigravity rule. Review and remove it manually, or reinstall tokenjuice antigravity first.`);
    }
...
export async function doctorAntigravityRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
    const existing = await readInstructionFile(resolvedRulePath);
    if (!existing.exists) {
        return {
