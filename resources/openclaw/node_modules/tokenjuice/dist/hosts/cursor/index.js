...
function isNativeWindowsCursorUnsupported() {
    return process.platform === "win32";
}
async function buildCursorHookCommand(options = {}) {
...
function sanitizeCursorHooksConfig(raw) {
    if (!isRecord(raw)) {
...
async function loadCursorHooksConfig(hooksPath) {
    try {
...
async function readCursorHooksConfig(hooksPath) {
    try {
        const rawText = await readFile(hooksPath, "utf8");
        const parsed = JSON.parse(rawText);
        return {
...
function findTokenjuiceCursorHookCommand(config) {
...
async function resolveCursorHostShell(toolInput) {
...
export async function installCursorHook(hooksPath = getDefaultHooksPath(), options = {}) {
...
export async function uninstallCursorHook(hooksPath = getDefaultHooksPath()) {
    let rawText;
    try {
...
export async function doctorCursorHook(hooksPath = getDefaultHooksPath(), options = {}) {
    if (isNativeWindowsCursorUnsupported()) {
...
export async function runCursorPreToolUseHook(rawText, wrapLauncher = "tokenjuice") {
    let payload;
    try {
