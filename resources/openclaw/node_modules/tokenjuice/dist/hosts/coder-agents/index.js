...
const TOKENJUICE_CODER_AGENTS_ADVISORY = "Coder Agents support is beta and skill-based; Coder discovers .agents/skills/tokenjuice/SKILL.md in workspace-attached chats, but tokenjuice does not intercept shell output.";
function isTokenjuiceCoderAgentsSkillText(text) {
...
async function chooseCoderAgentsBackupPath(skillPath) {
...
async function resolveSafeProjectSkillPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use Coder Agents skill ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
...
function buildCoderAgentsSkill({ restoreBackupSuffix } = {}) {
    return [
        ...
        ...buildTokenjuiceGuidanceBullets({
            wrapBullet: "- When Coder Agents runs terminal commands likely to produce long output, prefer `tokenjuice wrap -- <command>`.",
        }),
...
export async function installCoderAgentsSkill(skillPath, options = {}) {
    ...
    if (existing.exists) {
        ...
        await writeTextFileWithoutBackup(resolvedSkillPath, buildCoderAgentsSkill({ restoreBackupSuffix: backupPath.slice(resolvedSkillPath.length) }));
        return { skillPath: resolvedSkillPath, backupPath };
    }
    await writeTextFileWithoutBackup(resolvedSkillPath, buildCoderAgentsSkill());
    return { skillPath: resolvedSkillPath };
}
export async function uninstallCoderAgentsSkill(skillPath, options = {}) {
    const resolvedSkillPath = await resolveSkillPath(skillPath, options);
    const existing = await readInstructionFile(resolvedSkillPath);
    if (!existing.exists || !isTokenjuiceCoderAgentsSkillText(existing.text)) {
...
export async function doctorCoderAgentsSkill(skillPath, options = {}) {
