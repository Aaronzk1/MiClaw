...
const TOKENJUICE_ELYRA_ADVISORY = "Elyra support is beta and skill-based; Elyra discovers .elyra/skills/tokenjuice/SKILL.md when loading project skills, but tokenjuice does not intercept shell output.";
const ELYRA_FRONTMATTER_PATTERN = /^---[ \t]*\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/u;
function isTokenjuiceElyraSkillText(text) {
    return text.includes(TOKENJUICE_ELYRA_OWNERSHIP_MARKER);
}
function readRestoreBackupSuffix(text) {
...
async function chooseElyraBackupPath(skillPath) {
...
async function writeTextFileWithoutBackup(filePath, text) {
...
async function resolveSafeProjectSkillPath(filePath, projectDir, realProjectDir = projectDir) {
    ...
    if (!isInsideOrEqual(realProjectDir, realParentDir)) {
        throw new Error(`cannot use elyra skill ${resolvedPath}; tokenjuice will not write through instruction directories outside ${realProjectDir}`);
    }
...
function getLeadingFrontmatter(text) {
...
function getInlineScalarValue(value) {
...
function collectFrontmatterIssues(text) {
...
function buildElyraSkill({ restoreBackupSuffix } = {}) {
    return [
        ...
        ...buildTokenjuiceGuidanceBullets({
            wrapBullet: "- When Elyra runs terminal commands likely to produce long output through its `bash` tool, prefer `tokenjuice wrap -- <command>`.",
        }),
...
export async function installElyraSkill(skillPath, options = {}) {
    ...
    if (existing.exists) {
        ...
        await writeTextFileWithoutBackup(resolvedSkillPath, buildElyraSkill({ restoreBackupSuffix: backupPath.slice(resolvedSkillPath.length) }));
        return { skillPath: resolvedSkillPath, backupPath };
    }
    await writeTextFileWithoutBackup(resolvedSkillPath, buildElyraSkill());
    return { skillPath: resolvedSkillPath };
}
export async function uninstallElyraSkill(skillPath, options = {}) {
    const resolvedSkillPath = await resolveSkillPath(skillPath, options);
    const existing = await readInstructionFile(resolvedSkillPath);
    if (!existing.exists || !isTokenjuiceElyraSkillText(existing.text)) {
...
export async function doctorElyraSkill(skillPath, options = {}) {
