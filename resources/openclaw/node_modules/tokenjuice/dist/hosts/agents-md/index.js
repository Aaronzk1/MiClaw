...
const TOKENJUICE_AGENTS_MD_ADVISORY = "AGENTS.md support is beta and instruction-based; it guides agents that read AGENTS.md but does not intercept tool output.";
...
async function resolveSafeProjectInstructionsPath(filePath, projectDir) {
    ...
    if (!isInsideOrEqual(projectDir, realParentDir)) {
        throw new Error(`cannot use AGENTS.md instructions ${filePath}; tokenjuice will not write through instruction directories outside ${projectDir}`);
    }
...
const TOKENJUICE_AGENTS_MD_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When running terminal commands from an agent that reads AGENTS.md, prefer `tokenjuice wrap -- <command>` for commands likely to produce long output.",
    }),
...
export async function installAgentsMdInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install agents-md`);
    }
...
export async function uninstallAgentsMdInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = await resolveInstructionsPath(instructionsPath, options);
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_AGENTS_MD_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall agents-md`);
    }
...
export async function doctorAgentsMdInstructions(instructionsPath, options = {}) {
