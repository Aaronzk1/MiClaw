...
const TOKENJUICE_AGENTSGE_ADVISORY = "agents.ge support is beta and rule-based; run `agents sync` after install so generated agent entrypoints receive the rule.";
...
export async function installAgentsGeRule(rulePath, options = {}) {
...
export async function uninstallAgentsGeRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
    const result = await removeInstructionFile(resolvedRulePath);
    return { rulePath: result.filePath, removed: result.removed };
}
export async function doctorAgentsGeRule(rulePath, options = {}) {
    const resolvedRulePath = rulePath ?? (await getDefaultRulePath(options));
    const existing = await readInstructionFile(resolvedRulePath);
    if (!existing.exists) {
        return {
