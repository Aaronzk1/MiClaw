...
const TOKENJUICE_CRUSH_ADVISORY = "Crush support is beta and skill-based; it guides command usage but does not intercept or rewrite shell output.";
...
function getDefaultSkillPath(options = {}) {
...
export async function uninstallCrushSkill(skillPath = getDefaultSkillPath()) {
...
export async function doctorCrushSkill(skillPath, options = {}) {
    const resolvedSkillPath = skillPath ?? getDefaultSkillPath(options);
    const existing = await readInstructionFile(resolvedSkillPath);
    if (!existing.exists) {
        return {
