...
const TOKENJUICE_LOCALCODE_ADVISORY = "LocalCode support is beta and installs a plugin tool/command for compacting provided terminal output; it does not intercept LocalCode's built-in shell output.";
function getLocalCodeHome(options = {}) {
    return options.homeDir || process.env.LOCALCODE_HOME || join(homedir(), ".localcode");
}
function getPluginDir(options = {}) {
...
async function rejectPluginDirectorySymlink(pluginDir) {
    try {
...
async function rejectPluginDirectorySymlinks(options = {}) {
...
async function rejectPluginFileSymlink(filePath) {
    try {
...
async function rejectPluginFileSidecarSymlinks(filePath) {
...
async function chooseLocalCodeBackupPath(filePath) {
...
function readRestoreBackupSuffix(text, markerPrefix) {
    const match = text.match(new RegExp(`^// ${markerPrefix.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}(\\.bak(?:\\.\\d+)?)$`, "mu"));
    return match?.[1];
}
function readRestoreBackups(indexText) {
...
function buildLocalCodePlugin({ manifestBackupSuffix, indexBackupSuffix, } = {}) {
    return [
...
export async function installLocalCodePlugin(options = {}) {
    ...
    if (hasTokenjuiceMarker(existingManifest.text, existingIndex.text)) {
        ...
        const manifestBackupPath = existingManifest.exists && !existingManifest.text.includes(TOKENJUICE_LOCALCODE_OWNERSHIP_MARKER)
            ? await chooseLocalCodeBackupPath(manifestPath)
            : undefined;
        const indexBackupPath = existingIndex.exists && !existingIndex.text.includes(TOKENJUICE_LOCALCODE_OWNERSHIP_MARKER)
    ...
    await writeTextFileWithoutBackup(manifestPath, TOKENJUICE_LOCALCODE_MANIFEST);
    await writeTextFileWithoutBackup(indexPath, buildLocalCodePlugin({
...
async function restoreOrRemoveOwnedPluginFile(filePath, existing, restoreBackupSuffix) {
...
export async function uninstallLocalCodePlugin(options = {}) {
    const pluginDir = getPluginDir(options);
    const manifestPath = getManifestPath(options);
    const indexPath = getIndexPath(options);
    await rejectPluginDirectorySymlinks(options);
...
function collectManifestIssues(text) {
    let manifest;
    try {
...
export async function doctorLocalCodePlugin(options = {}) {
    const pluginDir = getPluginDir(options);
    const manifestPath = getManifestPath(options);
    const indexPath = getIndexPath(options);
    let manifest;
