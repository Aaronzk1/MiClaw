...
async function getDefaultToolPath(options = {}) {
...
export async function installOpenWebUITool(toolPath, options = {}) {
...
export async function uninstallOpenWebUITool(toolPath, options = {}) {
    const resolvedToolPath = toolPath ?? await getDefaultToolPath(options);
    ...
    if (existing.text !== TOKENJUICE_OPENWEBUI_TOOL) {
        throw new Error(`refusing to remove ${resolvedToolPath}; it does not match the current tokenjuice Open WebUI tool source. Review and remove it manually, or reinstall tokenjuice openwebui first.`);
    }
    const result = await removeInstructionFile(resolvedToolPath);
    return { toolPath: result.filePath, removed: result.removed };
}
export async function doctorOpenWebUITool(toolPath, options = {}) {
    const resolvedToolPath = toolPath ?? await getDefaultToolPath(options);
    const existing = await readInstructionFile(resolvedToolPath);
    if (!existing.exists) {
        return {
