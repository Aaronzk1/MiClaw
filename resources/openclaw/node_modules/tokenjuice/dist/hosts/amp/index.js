...
const AMP_INSTRUCTION_FILENAMES = ["AGENTS.md", "AGENT.md", "CLAUDE.md"];
...
function isInsideOrEqualPath(childPath, parentPath) {
    const relativePath = relative(parentPath, childPath);
    return relativePath === "" || (!relativePath.startsWith("..") && !isAbsolute(relativePath));
}
async function getTokenjuiceInstructionPathsInDir(dir, options = {}) {
...
async function findParentTokenjuiceInstructionPaths(startDir, boundaryDir, options = {}) {
    let current = resolve(startDir);
...
function shouldScanProjectTree(options) {
    return options.scanProjectTree !== false;
}
async function findProjectTokenjuiceInstructionPaths(projectDir, options = {}) {
...
async function getAmpInstructionPathForDir(dir) {
...
async function getDefaultInstructionPaths(options = {}, preferExistingTokenjuiceBlocks = false) {
    const explicitProjectDir = getExplicitProjectDir(options);
    if (explicitProjectDir) {
...
async function getDefaultUninstallInstructionPaths(options = {}) {
    const explicitProjectDir = getExplicitProjectDir(options);
    if (explicitProjectDir) {
        const projectDir = resolve(explicitProjectDir);
    ...
    const existing = uniquePaths([
        ...await findParentTokenjuiceInstructionPaths(process.cwd(), projectDir, { includeShadowedFallbacks: true }),
...
export async function installAmpInstructions(instructionsPath, options = {}) {
    ...
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
        ...
        if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
            throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install amp`);
        }
    }
    const results = [];
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
...
export async function uninstallAmpInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPaths = instructionsPath ? [instructionsPath] : await getDefaultUninstallInstructionPaths(options);
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
        const existing = await readInstructionFile(resolvedInstructionsPath);
        const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_AMP_BLOCK_CONFIG);
        if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
            throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall amp`);
        }
    }
    const results = [];
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
...
async function inspectAmpInstructionsPath(resolvedInstructionsPath) {
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_AMP_BLOCK_CONFIG);
...
export async function doctorAmpInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPaths = instructionsPath ? [instructionsPath] : await getDefaultInstructionPaths(options, true);
    const reports = [];
    for (const resolvedInstructionsPath of resolvedInstructionsPaths) {
