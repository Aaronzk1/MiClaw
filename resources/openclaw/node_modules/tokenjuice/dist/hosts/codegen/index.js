...
const TOKENJUICE_CODEGEN_ADVISORY = "Codegen support is beta and rule-file based; Codegen still owns command execution.";
...
const TOKENJUICE_CODEGEN_BLOCK = [
    ...
    ...buildTokenjuiceGuidanceBullets({
        wrapBullet: "- When running terminal commands through Codegen agents, prefer `tokenjuice wrap -- <command>` for commands likely to produce long output.",
    }),
...
export async function installCodegenInstructions(instructionsPath, options = {}) {
    ...
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely repair malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice install codegen`);
    }
...
export async function uninstallCodegenInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = instructionsPath ?? (await getDefaultInstructionsPath(options));
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_CODEGEN_BLOCK_CONFIG);
    if (existing.exists && hasMalformedMarkerStructure(existing.text, markerState.completeBlockCount)) {
        throw new Error(`cannot safely uninstall malformed tokenjuice markers in ${resolvedInstructionsPath}; remove the dangling marker manually, then rerun tokenjuice uninstall codegen`);
    }
...
export async function doctorCodegenInstructions(instructionsPath, options = {}) {
    const resolvedInstructionsPath = instructionsPath ?? (await getDefaultInstructionsPath(options));
    const existing = await readInstructionFile(resolvedInstructionsPath);
    const markerState = inspectMarkerDelimitedBlock(existing.text, TOKENJUICE_CODEGEN_BLOCK_CONFIG);
