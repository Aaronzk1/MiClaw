...
async function startFromConfig(config, overrides) {
	const logger = new require_logger.Logger("info");
	const llmock = new require_llmock.LLMock({
	...
	if (config.llm?.fixtures) {
		const fixturePath = node_path.resolve(config.llm.fixtures);
		if (node_fs.statSync(fixturePath).isDirectory()) llmock.loadFixtureDir(fixturePath);
		else llmock.loadFixtureFile(fixturePath);
	}
	if (config.mcp) {
		const mcpConfig = config.mcp;
		const mcp = new require_mcp_mock.MCPMock({ serverInfo: mcpConfig.serverInfo });
	...
	if (config.a2a) {
		...
		const a2a = new require_a2a_mock.A2AMock();
...
//# sourceMappingURL=config-loader.cjs.map