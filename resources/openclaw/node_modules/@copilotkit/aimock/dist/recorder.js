...
function persistFixture(opts) {
	...
	if (isSnapshotMode) {
		...
		if (!slug) {
			...
			filepath = path.join(fixturePath, `${providerKey}-${timestamp}-${crypto$1.randomUUID().slice(0, 8)}.json`);
		} else {
			filepath = path.join(fixturePath, slug, `${providerKey}.json`);
			mergeExisting = true;
		}
	} else {
		const timestampFile = `${providerKey}-${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${crypto$1.randomUUID().slice(0, 8)}.json`;
		filepath = fixture.match.context ? path.join(fixturePath, fixture.match.context, timestampFile) : path.join(fixturePath, timestampFile);
	}
	const fileWarnings = [...isEmptyMatch ? ["Empty match criteria — this fixture will not match any request"] : [], ...warnings];
	try {
		fs.mkdirSync(path.dirname(filepath), { recursive: true });
		let fileContent;
		if (mergeExisting && fs.existsSync(filepath)) try {
			fileContent = { fixtures: [...JSON.parse(fs.readFileSync(filepath, "utf-8")).fixtures ?? [], fixture] };
		} catch (mergeErr) {
...
async function proxyAndRecord(req, res, request, providerKey, pathname, fixtures, defaults, rawBody, options) {
	...
	try {
		...
		writeErrorResponse(res, 502, JSON.stringify({ error: {
...
function buildFixtureMatch(request, recordConfig) {
	...
	if (request.model) match.model = normalizeModelName(request.model, recordConfig?.recordFullModelVersion) ?? request.model;
...
function buildFixtureMetadata(request) {
	...
	if (systemTexts) meta.systemHash = crypto$1.createHash("sha256").update(systemTexts).digest("hex").slice(0, 8);
	if (request.tools && request.tools.length > 0) meta.toolsHash = crypto$1.createHash("sha256").update(JSON.stringify(request.tools)).digest("hex").slice(0, 8);
	return Object.keys(meta).length > 0 ? meta : void 0;
}

//#endregion
export { buildFixtureMatch, persistFixture, proxyAndRecord };
