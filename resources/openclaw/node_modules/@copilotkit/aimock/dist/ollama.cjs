...
async function handleOllama(req, res, raw, fixtures, journal, defaults, setCorsHeaders) {
	...
	if (!fixture) {
		...
		if (defaults.record) {
			const outcome = await require_recorder.proxyAndRecord(req, res, completionReq, "ollama", urlPath, fixtures, defaults, raw);
...
async function handleOllamaGenerate(req, res, raw, fixtures, journal, defaults, setCorsHeaders) {
	...
	if (require_helpers.isToolCallResponse(response) || require_helpers.isContentWithToolCallsResponse(response)) {
		journal.add({
			method: req.method ?? "POST",
			path: urlPath,
			headers: require_helpers.flattenHeaders(req.headers),
