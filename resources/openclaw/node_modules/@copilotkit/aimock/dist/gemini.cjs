...
function resolveAudioInlineData(audio) {
	if (typeof audio.audio === "string") return {
		mimeType: require_helpers.formatToMime(audio.format ?? "mp3"),
		data: audio.audio
	};
	return {
...
async function handleGemini(req, res, raw, model, streaming, fixtures, journal, defaults, setCorsHeaders, providerKey = "gemini") {
	...
	if (!fixture) {
		...
		if (defaults.record) {
			const outcome = await require_recorder.proxyAndRecord(req, res, completionReq, providerKey, path, fixtures, defaults, raw);
