...
async function handleCohere(req, res, raw, fixtures, journal, defaults, setCorsHeaders) {
	...
	if (!fixture) {
		...
		if (defaults.record) {
			const outcome = await require_recorder.proxyAndRecord(req, res, completionReq, "cohere", req.url ?? "/v2/chat", fixtures, defaults, raw);
