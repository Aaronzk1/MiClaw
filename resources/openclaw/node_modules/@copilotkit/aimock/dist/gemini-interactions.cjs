...
async function handleGeminiInteractions(req, res, raw, fixtures, journal, defaults, setCorsHeaders) {
	...
	try {
		...
		require_sse_writer.writeErrorResponse(res, 400, JSON.stringify(buildInteractionsErrorResponse(`Malformed JSON body: ${detail}`, "INVALID_ARGUMENT")));
		return;
	}
	...
	if (!fixture) {
		if (require_helpers.resolveStrictMode(defaults.strict, req.headers)) {
			...
			require_sse_writer.writeErrorResponse(res, strictStatus, JSON.stringify(buildInteractionsErrorResponse(strictMessage, "UNAVAILABLE")));
			return;
		}
		if (defaults.record) {
			const outcome = await require_recorder.proxyAndRecord(req, res, completionReq, "gemini-interactions", urlPath, fixtures, defaults, raw);
		...
		require_sse_writer.writeErrorResponse(res, 404, JSON.stringify(buildInteractionsErrorResponse("No fixture matched", "NOT_FOUND")));
		return;
	}
	...
	require_sse_writer.writeErrorResponse(res, 500, JSON.stringify(buildInteractionsErrorResponse("Fixture response did not match any known type", "INTERNAL")));
}

//#endregion
exports.geminiInteractionsToCompletionRequest = geminiInteractionsToCompletionRequest;
exports.handleGeminiInteractions = handleGeminiInteractions;
exports.resetEventIdCounter = resetEventIdCounter;
exports.resetInteractionCounter = resetInteractionCounter;
//# sourceMappingURL=gemini-interactions.cjs.map