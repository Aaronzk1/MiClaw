...
interface MCPConfigTool extends MCPToolDefinition {
  result?: string;
}
interface MCPConfigResource {
  uri: string;
  name: string;
  mimeType?: string;
  description?: string;
...
interface MCPConfigPrompt extends MCPPromptDefinition {
  result?: {
    messages: Array<{
      role: string;
      content: {
...
interface A2AConfigAgent extends A2AAgentDefinition {
...
declare function loadConfig(configPath: string): AimockConfig;
declare function startFromConfig(config: AimockConfig, overrides?: {
  port?: number;
  host?: string;
}): Promise<{
  llmock: LLMock;
...
//#endregion
export { AimockConfig, loadConfig, startFromConfig };
//# sourceMappingURL=config-loader.d.cts.map