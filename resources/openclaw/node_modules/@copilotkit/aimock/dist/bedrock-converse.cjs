...
async function handleConverse(req, res, raw, modelId, fixtures, journal, defaults, setCorsHeaders) {
	...
	if (!fixture) {
		...
		if (defaults.record) {
			const outcome = await require_recorder.proxyAndRecord(req, res, completionReq, "bedrock", urlPath, fixtures, defaults, raw);
