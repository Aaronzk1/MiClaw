...
if (Number.isNaN(journalMax) || !Number.isInteger(journalMax) || journalMax < 0) {
	console.error(`Invalid journal-max: ${values["journal-max"]} (must be a non-negative integer; 0 or omitted = unbounded)`);
	process.exit(1);
}
...
if (Number.isNaN(fixtureCountsMax) || !Number.isInteger(fixtureCountsMax) || fixtureCountsMax < 0) {
	console.error(`Invalid fixture-counts-max: ${fixtureCountsMaxStr} (must be a non-negative integer; 0 = unbounded)`);
	process.exit(1);
}
const upstreamTimeoutMsStr = values["upstream-timeout-ms"];
let upstreamTimeoutMs;
if (upstreamTimeoutMsStr !== void 0) {
	upstreamTimeoutMs = Number(upstreamTimeoutMsStr);
	if (!Number.isFinite(upstreamTimeoutMs) || upstreamTimeoutMs <= 0) {
		console.error(`Invalid upstream-timeout-ms: ${upstreamTimeoutMsStr} (must be a positive finite number)`);
		process.exit(1);
	}
}
const bodyTimeoutMsStr = values["body-timeout-ms"];
let bodyTimeoutMs;
if (bodyTimeoutMsStr !== void 0) {
	bodyTimeoutMs = Number(bodyTimeoutMsStr);
	if (!Number.isFinite(bodyTimeoutMs) || bodyTimeoutMs <= 0) {
		console.error(`Invalid body-timeout-ms: ${bodyTimeoutMsStr} (must be a positive finite number)`);
		process.exit(1);
	}
}
