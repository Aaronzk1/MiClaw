...
async function handleResponses(req, res, raw, fixtures, journal, defaults, setCorsHeaders) {
	...
	if (!fixture) {
		...
		if (defaults.record) {
			const outcome = await require_recorder.proxyAndRecord(req, res, completionReq, "openai", req.url ?? "/v1/responses", fixtures, defaults, raw);
...
//#endregion
exports.buildContentWithToolCallsStreamEvents = buildContentWithToolCallsStreamEvents;
