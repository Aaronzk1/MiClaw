...
async function loadPrettier() {
	return loadCached("prettier", async () => {
		const prettier = await import("./prettier-Cy-lZk4k.js");
...
async function setupTailwindPlugin(options) {
	CACHES.tailwindPlugin ??= await loadCached("tailwindPlugin", () => import("./dist-CJyFMFm2.js"));
	options.plugins ??= [];
...
async function sortTailwindClasses({ classes, options }) {
	CACHES.tailwindSorter ??= await loadCached("tailwindSorter", () => import("./sorter-CHFUGfz-.js"));
...
async function setupSveltePlugin(options) {
	CACHES.sveltePlugin ??= await loadCached("sveltePlugin", async () => await import("./plugin-C4twtOn4.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)));
	options.plugins ??= [];
